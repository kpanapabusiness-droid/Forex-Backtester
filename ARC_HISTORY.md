...
### Post-CC_07 anchor footnote (warmup convention)

> **v3 backtester anchor numbers reflect full-history warmup.** Per CC_07's anchor regression + warmup bisect (2026-05-22), the canonical v3 anchor is the A1 architecture path which evaluates signal-class features (kijun, atr, kijun_d1 exit predicate, H1 CIR) once on the full panel before slicing for per-fold driver loops. The pre-CC_07 `KH24FoldRunner` used a 30-day pre-OOS panel slice as signal-eval warmup, which NaN-masked `kijun(26)` on the first OOS bars of folds whose start dates landed near weekend/holiday clusters (F2 2021-07, F3 2022-04). Bisect confirmed: running the legacy path with `warmup_days=365` reproduces the A1 numbers byte-identically on all 7 anchor folds.
>
> The post-CC_07 v3 anchor fold-by-fold table lives in [docs/BACKTESTER_ARCHITECTURE.md §B](../docs/BACKTESTER_ARCHITECTURE.md); the pre-CC_07 legacy (30-day-warmup) numbers are preserved in §B.1 for reference. The PUBLISHED (5ers MT5 live deployment) numbers above — Pub ROI / Pub DD / Pub trades — are **unchanged** by CC_07. They reflect the live system's broker feed and execution lineage, not the HistData backtester. The live deployment on Contabo VPS / 5ers remains locked.
>
> Affected folds: F2 Pub +9.63% → v3 A1 +3.17% (legacy 30-day was +4.58%); F3 Pub +11.90% → v3 A1 +6.69% (legacy 30-day was +5.64% with 30 trades vs A1's 29). F1, F4, F5, F6, F7 byte-identical between the two warmup conventions. Spread audit + MTM-vs-closed-trade DD adjustments (see "Post-Arc-4 audit reconciliation" above) apply on top of either warmup convention and explain the residual divergence vs Pub.
