...
> **Amendment 8 (2026-06-02):** Standing run-rules — the DEFAULT basis for every future arc (every value overridable per-run, every run must RECORD what it used) — plus a mandatory provenance stamp and a decorrelation deployment-value gate. Sizing default flips to **fixed-initial** (floating-equity demoted to reference/comparison — supersedes Amendment 7(b)'s floating-as-deployable mandate on a going-forward basis; Arc 10's own canonical framing reconciled to fixed-initial). Default timeframe EET; daily DD **`daily_ref=initial`, resetting each broker day** (`day_start` selectable; the non-resetting `static_noreset` mode is quarantined — never canonical); max DD trailing(plan)+static(enforce) both reported; governed costs + 1.5× spread + $5/lot RT + swaps OFF + 0.5 pip slip; no CAGR, holdout per-year, worst-fold WFO sole judge. Every WFO table carries a fixed-schema provenance header (a run is flagged INVALID if absent — same discipline as the determinism sha-check). No archive file — Amendment 8 documented inline at §3 only. See §3 "Amendment 8" and `ARC_RUN_TEMPLATE.md`.
>
> **Reset amendment — Engine of record (2026-06-02):** the fast path-replay scorer (`simulate_path` / precomputed realised-R) is **RETIRED** for a gate-fidelity defect (it skipped pre-partial stops and flattered every gate it scored — see `docs/ARC_10_GATE_FIDELITY_DEFECT.md`). **`MultiPairBacktester` (`core/sim/`) is now the SOLE engine that may score a trade at Step 5** (two-stage triage — §2 Step 5). The **take-the-loss invariant** is locked (§1). The **Arc-10-specific and risk-tier-specific framing in Amendments 7 and 8 (sizing-basis adjudication, tier sweeps, Arc-10 reconciliation) is SUPERSEDED** — there is no deployable system and no live risk tier on the clean base (deployable-system count = 0). The *methodology* parts of those amendments (provenance stamp, decorrelation gate, determinism discipline, EET daily-DD boundary, ex-ante population, causal audit, AUC-gated architecture selection) remain in force.
>
...
- **Single truth engine.** `MultiPairBacktester` (`core/sim/`) is the ONLY engine that may score a trade for a gate. No code path scores P&L from a precomputed shortcut (the fast replay was retired 2026-06-02 for a gate-fidelity defect — see `docs/ARC_10_GATE_FIDELITY_DEFECT.md`).
- **Take-the-loss.** Any stop breach at or before the +1R partial/TP bar resolves to −1R (full position); a same-bar stop is SL-first; ambiguity never resolves to a win. Pinned by `tests/sim/test_take_the_loss_invariant.py` (CI-gated).
...
**Gate engine (mandatory, 2026-06-02):** every Step 5 P&L number is produced by walking bars through `MultiPairBacktester` (`core/sim/`) — the SOLE gate engine. The fast path-replay / precomputed-realised-R path is retired (gate-fidelity defect; see `docs/ARC_10_GATE_FIDELITY_DEFECT.md`). The stop is honoured SL-first per the §1 take-the-loss invariant. Because the truth engine walks bars, Step 5 uses a **two-stage triage** to stay tractable: (1) run candidates on a small set of representative folds on the truth engine and rank; (2) run only the top survivors on the full WFO on the truth engine. There is no fast-replay shortcut at either stage.
...
> **DEFAULT updated by Amendment 8 (2026-06-02):** the standing daily-DD **default** is now `daily_ref="initial"` (fixed-$ off the tier's initial balance), with this `day_start` basis (`daily_ref="day_start"`) retained as a **selectable** alternative (the 5ers basis). Both reset each broker day. The non-resetting `static_noreset` mode is quarantined (see §"Amendment 8"). This section's mechanics (per-day series persisted, resetting each broker day, no linear scaling of breach counts) are unchanged — only which baseline is the default.
...
**(b) Sizing must match the live execution basis, and be validated against it.** **[Superseded on the sizing-default by Amendment 8 (2026-06-02):** the default deployable basis is now **fixed-initial**, with floating-equity retained as a reference/comparison run; the Arc 10 deployed EA sizes fixed-initial. The portfolio-level DD requirement of (a) and the both-references DD reporting below are unchanged. The floating-vs-fixed framing in the rest of this clause is retained as the historical Arc 10 reconstruction-validation record.] The Step 5 run sizes each entry as the deployed EA does — `risk_amount = ACCOUNT_EQUITY × r_base`, re-read per entry — and linear / closed / floating runs are retained as reconstruction-validation steps (each must reproduce the same portfolio worst-fold trailing DD under a `mult ≡ r_base` + zero-cost reduction). For Arc 10, the floating-equity run was the original EA-faithful precedent and is retained as the reference comparison; the procyclical concurrency tail it measured (entries size larger when the open book is up, into a possible reversal) was benign (+0.16pp worst-fold trailing DD). Where a strategy has no live EA yet, the simulator stands in and must be re-validated against the EA once built.
...
Precedent: Arc 10 v3.0.2 — `results/l_arc_10_v3.0.2_ea_faithful/` (floating, reference) + `results/l_arc_10_v3.0.2_canonical/` (fixed-initial, canonical) + `arc_10/02_validation/07_canonical_wfo.md`. Amendment 7 is documented inline here — no separate archive file.

### Amendment 8 — standing run-rules (default basis) + provenance stamp + decorrelation gate

Effective 2026-06-02. Applies to **all future arcs**. Defines the DEFAULT basis under which an arc runs. **Every value below is a DEFAULT and is overridable per-run; every run MUST RECORD what it actually used** (via the §8.2 provenance stamp). No archive file — documented inline here.

**8.1 — Default run basis (overridable per-arc, always recorded):**

- **Timeframe:** EET (default). UTC only if explicitly stated and justified in the stamp.
- **Sizing:** **fixed-initial balance** (default; `1R = r_base` of the tier's initial balance, held flat). Floating-equity selectable as a reference/comparison run. (Supersedes Amendment 7(b)'s floating-as-deployable default; the Arc 10 deployed EA sizes fixed-initial.)
- **Daily DD:** **`daily_ref="initial"`** (default) — a fixed-$ budget = % of the tier's **initial** balance, with the daily window **RESETTING each broker day** (matches the FundedNext basis + the deployed EA `Daily_DD_Basis=INITIAL`, mandatory daily reset). `daily_ref="day_start"` selectable (5% of the day's starting equity/balance — the 5ers basis; also resetting). **The non-resetting `static_noreset` mode is QUARANTINED — never canonical:** a daily measure that never resets froze Arc 10 F5 2014 / F6 2015 at −5.99% / −5.45% (artefactually) where the resetting basis recovers them to +28.91% / +20.77% with 0 daily fires. Whatever `daily_ref` is chosen, the daily window MUST reset each broker day.
- **Max DD:** **trailing for PLANNING + static for ENFORCEMENT, both reported** (`total_ref` records the pair). The asymmetry is DELIBERATE: **trailing** = "safe from whatever point we begin" (the fresh-tier / scale-reset worst case, where the peak is the entry point); **static** = what the prop firm actually enforces (floor = initial − limit, never trails). Because `trailing ≥ static` by construction, **passing on trailing ⇒ safe on static** — trailing is the conservative planning anchor, static is the deployment-binding number.
- **Costs:** governed (3.5% daily halt / 4.5% daily close-all; 7% total halt / 8% total close-all) + 1.5× spread + $5/lot RT commission + swaps OFF + 0.5 pip slippage × n_fills. (FundedNext cost cell 5 basis.)
- **No CAGR anywhere.** Holdout reported **per-year**. Partial years (e.g. the current calendar year) are reported **raw, flagged as partial, never annualised.**
- **Worst-fold WFO is the sole deployment judge.** Mean-fold and full-period figures are context only.

**8.2 — Mandatory provenance stamp (every WFO table / saved run):**

Every WFO output MUST carry the header below; a run is **INVALID / flagged** if it is absent (same discipline as the determinism sha-check — it converts "what conditions was this run under?" from a recurring question into a stamped fact, the durable fix for basis-confusion). Fixed schema:

```
BASIS:   <EET|UTC> | governed (3.5/4.5 daily, 7/8 total) | 1.5x spread | $5/lot RT | swaps OFF | 0.5 slip
SIZING:  <fixed-initial|floating>
DD REFS: daily=<initial|day_start> (resetting) | max=<trailing(plan)+static(enforce)|...>
RISK:    r_base = X%
FRAME:   sha <...> | N pairs | folds + holdout-per-year (no CAGR)
DECORRELATION: <co-fire vs deployed arcs, or N/A for first arc>
```

**8.3 — Decorrelation gate (new arcs vs deployed):**

Any new arc intended to take capital **beyond the already-deployed set** must be assessed for **co-fire / return correlation vs deployed arcs** (currently Arc 10). A deployable-but-correlated arc does **not** expand capacity — the copy-trading constraint and the absence of diversification benefit mean a correlated survivor competes for the same slot rather than adding one. Report co-fire rate and/or return correlation as a Step 4–5 output. **Decorrelation is a deployment-value gate, not just a standalone-edge gate.** First-arc / sole-system runs record `DECORRELATION: N/A (first arc)` in the stamp.
