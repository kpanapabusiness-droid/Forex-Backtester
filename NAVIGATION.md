...
| **Run autonomous discovery** | [`discovery/DISCOVERY_PROTOCOL.md`](./discovery/DISCOVERY_PROTOCOL.md) (authoritative — start here) → [`discovery/README.md`](./discovery/README.md) (one-screen: arc loop, chat-range, STOP) → [`discovery/TOOL_REGISTRY.md`](./discovery/TOOL_REGISTRY.md) (canonical measurement entry point — call, never re-roll — + the BUILT experiment-tool registry) → [`docs/DISCOVERY_COUNCIL_SKILL.md`](./docs/DISCOVERY_COUNCIL_SKILL.md) (the council, invoked via `/llm-council-discovery`). **§0 precondition:** the engine sweep must read clean ([`HONEST_ENGINE_SWEEP.md`](./HONEST_ENGINE_SWEEP.md)) — it does as of 2026-06-05 (incl. the short-side re-read addendum). Every arc ends in one **disposition: PASS** (all-folds-positive IS+OOS) / **PORTFOLIO** (mean-positive, not all-folds-positive — a decorrelated component) / **KILL** (§11). Short-side capability is enabled (PR #273; sweep SAFE). |
