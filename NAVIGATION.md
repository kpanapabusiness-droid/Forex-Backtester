...
| **Start a new arc** | [`ARC_RUN_TEMPLATE.md`](./ARC_RUN_TEMPLATE.md) (self-run spec — copy, fill blanks) → [`L_PROTOCOL.md`](./L_PROTOCOL.md) (methodology; Step 5 = `MultiPairBacktester` sole gate engine). Supply the signal idea + any non-default settings. A separate self-running discovery protocol is authored elsewhere — do not create it here. |
