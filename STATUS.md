...
> Last updated: 2026-05-19 — **Arc 8** (PR-HHHL long) CLOSED 2026-05-18 HALT_DEPLOYMENT; **Arc 9** CLOSED STEP_4_KILL_REAFFIRMED after producer-leak patch (v2.x §8 D1 feature-budget WITHDRAWN; §3 threshold-grid WEAKENED; NEW Amendment 1 = producer-level causal audit); **Arc 10** (DLR) closed `STEP_4_HALT` 2026-05-18 on `claude/charming-mcnulty-8160e0` — first arc end-to-end under v2.3; post-closure EXP-01–06 + WFO pair (base + oracle c1) complete; oracle Sharpe 4.61 vs base −1.29; EXP-05 cross-arc V-shape pool (Arc 7 c3 + Arc 10 c1) AUC 0.6348 closest to gate; corrections: Arc 6 reclass Stepwise (per EXP-05), fold-2 date 2023-07 → 2024-06 (per EXP-04); Open-06 WEAKENED, Open-04 DEFERRED. **Arc 11** (SHB long 4H) CLOSED-HALT per §16a Path A (second capturable-not-extractable closure). Engine PR `feat/open-24-pre-t-sl-per-archetype` (Open-24) merged 2026-05-19 (PR #146). Prior context: 2026-05-18 L_ARC_PROTOCOL v2.3 amendment landed (Step 5 cross-fold stability removed; Step 6 WFO renumbered as Step 5; Open-22/23/24 closed in protocol); v2.2 amendment landed earlier same day; Arcs 4, 5, 6, 7 all closed. KH-24 live deployment unchanged.
...
- Next chat task: analyst populates `results/ARC_QUEUE.md` Unrun section with Arc 9+ signal specs (Arc 8 closed HALT_DEPLOYMENT 2026-05-18; see Recent Closures and `results/l_arc_8/ARC_8_CLOSURE.md`); v2.4 protocol amendment design (§1.5 entry-separability gate) — see `PROTOCOL_IMPROVEMENT_BACKLOG.md`.

---

## Current Phase

**Arc 10 closed `STEP_4_HALT` 2026-05-18 on `claude/charming-mcnulty-8160e0`** (DLR — D1 swing-low rejection long, out-of-registry `signal_spec_d1_swing_low_rejection_long_v0.1.md`). First arc to run end-to-end under v2.3 (5-step pipeline, halt at end of Step 4). Steps 1-3 PASS clean; Step 4 disjunctive §8 near-miss FAIL (c1 V-shape recovery: E AUC 0.6296 margin −0.0204; D1 AUC 0.5897 margin −0.0103). §16a Path A compound reading applied; strict reading available (compound-vs-strict ambiguity flagged for v2.4). Post-closure research (over §16a, chat-direction): EXP-01–06 experimentation pass + WFO pair (base + oracle c1) complete — oracle Sharpe 4.61 vs base −1.29 (gap +5.90 passes material thresholds; explicit DO NOT DEPLOY because oracle is upper bound). EXP-01 P(realisable AUC ≥ 0.65) under bootstrap = 12.5%. EXP-02 identifies `L1_minus_L0_atr` as load-bearing HTF feature (116% of LOO drop). EXP-05 cross-arc V-shape pool (Arc 7 c3 + Arc 10 c1) AUC 0.6348 — strongest evidence V-shape is cross-arc deployable. Closure at `docs/arc_results/ARC_10_RESULT.md`. Material corrections: Arc 6 reclass Stepwise (was V-shape); fold-2 date 2023-07/2024-06 (was Q2 2022). Open-06 (AUC threshold) WEAKENED; Open-04 (external features) DEFERRED pending Arc 8/9/11 reproduction. Next dispatch rank order: Reverse-FE diagnostic (1), cross-arc clusterifier build (2), filter-path probe (3), v2.4 calibration packet (4). Arcs 8/9/11 in parallel CC sessions — `results/ARC_QUEUE.md` not modified at any Arc 10 commit.
...
**Arcs 4-10 closures (Arcs 8/9/11 in parallel CC sessions):**
- **Arc 10** — DLR (D1 swing-low rejection long, `signal_spec_d1_swing_low_rejection_long_v0.1.md`). Closed 2026-05-18 `STEP_4_HALT` per §16a Path A. First arc end-to-end under v2.3. c1 V-shape recovery near-miss on disjunctive §8 (E 0.6296 / D1 0.5897). Post-closure: experimentation pass EXP-01–06 + WFO pair (base / oracle c1) — oracle Sharpe 4.61, gap +5.90, BUILD recommendation with explicit DO NOT DEPLOY (oracle is upper bound; EXP-01 realisable P(AUC≥0.65)=12.5%). EXP-05 cross-arc pool AUC 0.6348 with Arc 7 c3. Corrections: Arc 6 reclass Stepwise; fold-2 date 2023-07/2024-06. `docs/arc_results/ARC_10_RESULT.md`.
...
| Arc queue | `results/ARC_QUEUE.md` — owned by parallel CC session running Arcs 8/9/11; this branch does not modify it |
...
| Current arc | None active — Arc 8 closed HALT_DEPLOYMENT 2026-05-18; Arc 9 closed STEP_4_KILL_REAFFIRMED 2026-05-19; Arc 10 closed STEP_4_HALT 2026-05-18; Arc 11 closed HALT 2026-05-18 |
...
| --- | --- | --- |
| Arc 9 (l_arc_9) | **CLOSED 2026-05-19, STEP_4_KILL_REAFFIRMED** | IB-trend compression-break long (`signal_spec_inside_bar_break_trend_long_v0.1.md`). First closed STEP_4_KILL 2026-05-18 (extractability FAIL — Pipeline E AUC 0.511, D1 AUC 0.626 with threshold-sweep recall 0.003); held-open cycle ran 7 diagnostic experiments through 2026-05-19. Held-open Pipeline E retry (+12 features, AUC 0.7508) generated a transient deployment-candidate picture (Candidate A worst-fold ann ROI +9.63%, Candidate B +20.68%, scaled-risk 1.0% recommendation). External producer-level audit detected lookahead in two D1 swing features (`d1_bars_since_swing_low`, `d1_bars_since_swing_high` — centred ±10-bar window suppressed only at frame tail). Causal patch (commit 5b6c547, confirmed-swing detector with 10-day lag): AUC collapses to 0.5190 (LGBM) / 0.5551 (RF), both below §8 gate 0.65. Forced WFO at patched classifier (commit 51457e6) confirms: Cand A full-data ROI −0.13% / DD 13.38%; Cand B +0.08% / DD 22.65%. Cohort verified deployable in Step 5 oracle (+39.45% worst-fold / 0.01% DD with post-hoc cluster identity) but **unreachable on causally-clean in-protocol features**. NEW forward-looking artefact: producer-level causal scope as standard audit dimension (raised as v2.x Amendment 1, highest priority). v2.x §8 D1 feature-budget expansion proposal WITHDRAWN; v2.x §3 threshold-grid replacement WEAKENED (Arc 7 calibration recovery becomes load-bearing). Closure: `results/l_arc_9/ARC_9_CLOSURE.md`. Incident: `results/l_arc_9/INCIDENT_2026_05_19_ARC_9_PRODUCER_LEAK.md`. |
| Arc 8 (l_arc_8) | HALT_DEPLOYMENT (2026-05-18) | PR-HHHL long (`signal_pullback_resume_hhhl_long_v0.1`). Steps 1-4 PASS — one archetype survived Step 4 (c1 V-shape recovery FG-weak, +2.59R admit-only mean). Step 5 WFO FAIL §10 ship gates: classifier admits 70-89% of full Step 1 OOS pool (c1 + c2 share entry-bar geometry); non-c1 admits with mean_r ≈ −0.3R dilute c1's +2.59R edge to negative aggregate. Pipeline E full-pool worst ROI −13.21% / max DD 15.58% / Sharpe 0.012; Pipeline D1 full-pool worst ROI −14.69% / max DD 19.02% / Sharpe 0.001. Three post-Step-5 diagnostics confirmed failure is STRUCTURAL: (a) entry-feature overlap diagnostic — c1 1-vs-rest AUC 0.547, precision@recall=0.60 = 0.149 (zero lift over base rate 0.133); (b) Path 1 post-entry confirmation t-sweep — c1 AUC climbs 0.547 → 0.755 (t=12) but precision@recall=0.60 plateaus at 0.274 (marginal, below 0.40 viable); (c) Path 2 mechanical signal-tightening — no filter satisfies c1_ret ≥ 0.80 ∧ c2_ret ≤ 0.30 ∧ pool ≥ 500 simultaneously. **3rd consecutive Open-22/23/24 admit-only-vs-deployment failure** (Arcs 4 RERUN, 5, 8) — pattern is framework-level, not arc-level. v2.4 §1.5 entry-separability gate proposed as Open-25 (`PROTOCOL_IMPROVEMENT_BACKLOG.md`). c1 archetype logged for Open-05 portfolio composition (admit-only Pipeline E Sharpe 1.44 / DD 1.00% / ROI 18.66%; Pipeline D1 Sharpe 1.14 / DD 0.54% / ROI 25.73% — tradeable in isolation, not routable from PR-HHHL signal alone). `pullback_depth_atr ≥ 1.0` filter improves aggregate Step 1 mean_r +68% (signal-design observation). Engine PR `feat/open-24-pre-t-sl-per-archetype` merged 2026-05-19 (PR #146). Closure doc: `results/l_arc_8/ARC_8_CLOSURE.md`. |
| Arc 10 (l_arc_10) | STEP_4_HALT (2026-05-18) | DLR — D1 swing-low rejection long (out-of-registry, `signal_spec_d1_swing_low_rejection_long_v0.1.md`). First arc end-to-end under v2.3. Steps 1-3 PASS clean; Step 4 disjunctive §8 near-miss FAIL: c1 V-shape recovery E AUC 0.6296 (margin −0.0204), D1 AUC 0.5897 (margin −0.0103). §16a Path A compound reading applied (single compound §8 gate fails, both numeric margins < 0.03, cohort size_fraction 0.284). Strict reading (two criteria → KILL) flagged for v2.4. Post-closure research over §16a (chat-direction): EXP-01–06 experimentation pass + WFO pair (base + oracle c1). EXP-01 bootstrap shows P(realisable AUC ≥ 0.65) = 12.5%. EXP-02 `L1_minus_L0_atr` carries 116% of HTF LOO drop. EXP-05 cross-arc V-shape pool (Arc 7 c3 + Arc 10 c1) AUC 0.6348 (gap −0.015 to gate). WFO oracle Sharpe 4.61 vs base −1.29 (gap +5.90, BUILD recommendation; explicit DO NOT DEPLOY because oracle is upper bound). Material corrections: Arc 6 reclassified Stepwise (not V-shape); fold-2 date 2023-07 → 2024-06 (not Q2 2022). Open-06 weakened; Open-04 deferred. `docs/arc_results/ARC_10_RESULT.md`. Commits: `362a085` (step 1) → `f39dcd9` (steps 2-3) → `994f642` (step 4 HALT) → `73ba1f0` (experiments) → `07e41c1` (WFO pair) → `0f89986` (closure relocation). Queue not touched (parallel CC session running Arcs 8/9/11). |
...
| Open-06 — AUC threshold recalibration | Arc 6 (originally), Arc 7, Arc 10 | WEAKENED | Arc 10 closure originally claimed three V-shape near-miss data points (Arc 6, 7, 10); EXP-05 reclassified Arc 6 as Stepwise (not V-shape). Clean V-shape pair (Arc 7 + Arc 10) requires deeper threshold relaxation than the previously-claimed three-arc relaxation (binding at ~0.536 via Arc 7 agg_c1_c3, not 0.600 via Arc 6+10 mixed-archetype pair). Reassess in v2.4. |
| Open-04 — external-feature commission | Arc 7, Arc 10 (EXP-06) | DEFERRED | EXP-06 on Arc 10 probed two candidates (D1 Kijun distance, session dummies) — both produced negative AUC delta on Arc 10 alone (small-n saturation: 25 baseline features + n=228). Single-arc evidence insufficient. Needs Arc 8/9/11 reproduction before commission decision. |
| §16a Path A disjunctive-gate ambiguity | Arc 10 | NEW (MEDIUM) | "Single criterion fail" wording unclear when a disjunctive Step 4 gate (E OR D1) fails on both. Compound reading (Step 4 as one §8 gate → HALT) vs strict reading (two numeric criteria → KILL) produces different dispositions. Arc 10 invoked compound. Formalise in v2.4; review Arc 6 / Arc 7 closures for consistency. |
| Reverse FE methodology framework | Arc 10 closure | NEW (LOW) | Activity class defined only in Arc 10 closure + post-closure dispatches. Lift to framework methodology doc (or create one). Includes anti-snooping protocol (pre-registered hypothesis catalog), hold-out conventions at small N, and routing rules to classifier vs filter paths. |
| Within-cluster heterogeneity probing | Arc 10 c1 | NEW (LOW) | Step 3 capturability composite 0.4934 on Arc 10 c1 hints at heterogeneity. Deep-V vs shallow-V sub-clustering may produce a cleaner-signature sub-type. Methodology candidate for Step 2 extension or as part of reverse FE Stage 1. |
| Q2-2022 misattribution audit | Arc 10 closure correction | NEW (LOW) | Arc 10's fold-2 was originally narrated as Q2 2022; EXP-04 confirmed actual window is 2023-07 → 2024-06. Possible same misattribution elsewhere. Audit closure docs across all arcs for "Q2 2022" references. |
| Filter-path validation regime | Arc 10 closure (rank #3 dispatch) | NEW (MEDIUM) | No precedent in current framework for validating a deterministic filter against post-filter trade-set P&L rather than classifier AUC. Define acceptance criteria (Sharpe, expectancy, max DD thresholds) before any filter-path probe dispatches downstream. |
| `L1_minus_L0_atr` cross-arc feature promotion | Arc 10 (EXP-02) | NEW (LOW) | EXP-02 single-feature LOO drop is 116% of HTF lift on Arc 10. Promote to standard feature catalog pending cross-arc reproduction (Arc 8 / 9 / 11 when available). |
| V-shape archetype as cross-arc abstraction | Arc 7 + Arc 10 (EXP-05 + WFO oracle) | NEW (MEDIUM) | EXP-05 pool AUC 0.6348 + WFO oracle Sharpe 4.61 establishes the archetype carries real OOS edge. Build deployable abstraction pending reverse-FE-fed classifier (rank #2 next dispatch). |
...
| Arc 10 (closed STEP_4_HALT; post-closure experimentation + WFO pair over §16a) | `results/l_arc_10/` (step1_verbatim, step2, step3, step4, experiments, wfo_base, wfo_oracle_c1, wfo_pair_synthesis.md) + `docs/arc_results/ARC_10_RESULT.md` |
