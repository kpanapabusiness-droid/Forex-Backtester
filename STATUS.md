...
> Last updated: 2026-05-19 — **Arc 9** CLOSED STEP_4_KILL_REAFFIRMED after producer-leak patch invalidated held-open Pipeline E retry's AUC 0.7508 (was a leak artefact; patched AUC 0.5190 below §8 gate); cohort verified deployable in Step 5 oracle but unreachable on causally-clean features; v2.x amendment proposal revised (§8 D1 feature-budget expansion WITHDRAWN; §3 threshold-grid replacement WEAKENED; NEW Amendment 1 = producer-level causal audit dimension as highest priority); closure `results/l_arc_9/ARC_9_CLOSURE.md`, incident `results/l_arc_9/INCIDENT_2026_05_19_ARC_9_PRODUCER_LEAK.md`. **Arc 11** (SHB long 4H) housekeeping landed: CLOSED-HALT per §16a Path A (best AUC 0.5728, margin 0.027); second confirmed capturable-not-extractable closure (pairs with Arc 6); 4 post-closure experimental sessions retired 8 directions and filed Pipeline DE + `min_observation_bars` as v2.4 amendment candidates; closure `results/l_arc_11/ARC_11_CLOSURE.md`. Sibling arcs 8/10 still in flight in parallel CC chats. Prior context: 2026-05-18 L_ARC_PROTOCOL v2.3 amendment landed (Step 5 cross-fold stability removed; Step 6 WFO renumbered as Step 5; Open-22/23/24 closed in protocol with engine PR pending for Open-24); v2.2 amendment landed earlier same day; Arcs 4, 5, 6, 7 all closed. KH-24 live deployment unchanged.
