...
> Last updated: 2026-06-02 — **Arc 10 analysis COMPLETE + documented; deployed both terminals (FundedNext + 5ers); EA FIX 1/2b + backtester alignment + FINAL CANONICAL WFO (#254) DONE.** Risk-surface work closed (canonical = **#254 deploy-faithful** run: fixed-initial sizing + `daily_ref=initial`-resetting, `results/l_arc_10_v3.0.2_final_canonical/`; floating EA-faithful run retained as relabelled reference). Remaining: live-only residuals + deferred items (see Round 10).
...
> **Note (2026-06-02, Round 10):** Round 9 established the floating EA-faithful run as canonical. The genuinely-final canonical is the **#254 deploy-faithful run** — **fixed-initial sizing + `daily_ref=initial`-resetting daily-DD** (matching the live EA post-FIX-2b), at `results/l_arc_10_v3.0.2_final_canonical/`. The floating run is retained as a relabelled reference. Round 9 rows are kept as historical record; the authoritative basis is now #254 per `07_canonical_wfo.md`.

### Round 10 — FINAL CANONICAL (#254) + Amendment 8 + self-run template + broker ref + full doc reconciliation (2026-06-02)

The engine/EA chain is **DONE/deployed both terminals** (FundedNext + 5ers): EA FIX 1 (fixed-initial sizing) + FIX 2b (daily-DD basis param + mandatory daily reset) + backtester daily_ref alignment (#253) + **FINAL CANONICAL WFO (#254)**. This pass closes the documentation against #254's numbers.

| Task | Status | PR / Tag |
|---|---|---|
| EA FIX 1-3 (FundedNext-aligned sizing, daily-DD anchor, ±5min news) | 🟢 | #249 |
| WFO fixed-initial path + FundedNext daily-DD anchor | 🟢 | #250 |
| EA FIX 2b — daily-DD basis parameter + mandatory daily reset | 🟢 | #252 |
| Backtester `daily_ref` aligned to deployed EA (resetting daily-DD) | 🟢 | #253 |
| **FINAL CANONICAL WFO** — fixed-initial + `daily_ref=initial`-resetting (0.40% DEPLOYABLE 13.77%/28.85%/7.73%/4.06%; 0.42/0.45 margin-check; F5/F6 recover) | 🟢 | **#254** |
| L_PROTOCOL Amendment 8 — fixed-initial default + `daily_ref=initial`-resetting default + provenance stamp + decorrelation gate (inline) | 🟢 | this PR |
| `ARC_RUN_TEMPLATE.md` — self-run spec (CC-auto vs analyst-stop split, gates, defaults block) | 🟢 | this PR |
| `BROKER_RULES.md` — per-broker measurement table (daily-DD: FundedNext INITIAL / 5ers DAY_START) + per-account EA inputs incl. `Daily_DD_Basis` (FundedNext cited; 5ers TBD-VERIFY) | 🟢 | this PR |
| Docs reconciled to #254 (`07_canonical_wfo.md` legacy 0.50%/floating prose cleaned; CLAUDE.md; floating retained as relabelled reference) | 🟢 | this PR |
| Memory: canonical-gate entry needs updating to #254 fixed-initial + daily_ref=initial (analyst handles separately) | 🔴 | flagged in PR body |

**Remaining (deferred):** per-terminal sizing-param confirmation (5ers `Risk_Per_Trade`); Arc 10 gate post-mortem; OPEN-001 residual #1 (unattended floor-fail alerting, #245); disaster-recovery floor-restart step; branch cleanup (close stale PR #251 — superseded by this pass; the #254-results branch).
...
|---|---|---|---|---|
| Arc 10 | PASS-VIABLE → PASS-DEPLOYABLE | **PASS-DEPLOYABLE** (#254 final canonical: fixed-initial + daily_ref=initial, 0.40% FundedNext; floating EA-faithful = reference) | #174 / re-eval #177 / v3.0.2 #214/#216/#217/#218 / canonical #243/#246 / final #254 / docs reconciled this PR | **Deployed both terminals at 0.40%** |
...
> **Analysis-complete, deployed both terminals at 0.40% as of 2026-06-02.** Operating tier is **0.40%** (#254 final canonical: fixed-initial + `daily_ref=initial`-resetting; floating EA-faithful retained as reference). Operational detail lives in `arc_10/04_runbook/`; start at `arc_10/START_HERE.md`. This TODO captures decision-gate / go-live status only; it references `arc_10/`, it does not duplicate it.
