...
LEADERBOARD_CSV = "leaderboard_c1_param.csv"
SURVIVORS_MD = "phaseC1_survivors.md"
...
def _apply_gates(
    ...
    """
    Apply Phase C.1 PASS/REJECT rules. Return (pass_reject, rejection_reason).
...
def build_leaderboard(wfo_runs_root: Path) -> pd.DataFrame:
    """
    Build Phase C.1 leaderboard from results/phaseC1/wfo_runs/<variant_id>/<run_id>/fold_XX/out_of_sample.
    One row per parameter variant with pass_reject, worst_fold_*, median_fold_*, rejection_reason.
    """
    ...
    rows: List[Dict[str, Any]] = []
    for var_dir in sorted(wfo_runs_root.iterdir()):
        if not var_dir.is_dir():
            continue
        variant_id = var_dir.name
        run_dir = _latest_run_dir(var_dir)
        if run_dir is None:
            rows.append(
                {
...
def _write_survivors_md(
    ...
    """Write phaseC1_survivors.md: PASS list with bullets, REJECT list with one-line reason."""
    survivors = leaderboard_df[leaderboard_df["pass_reject"] == "PASS"]
    rejects = leaderboard_df[leaderboard_df["pass_reject"] == "REJECT"]

    lines: List[str] = [
        "# Phase C.1 — C1 Parameter Sensitivity Survivors",
        "",
        "## PASS",
        "",
    ]
    ...
    for _, row in rejects.iterrows():
        ...
        lines.append(f"- **{row['variant_id']}**: {reason}")
    lines.append("")
    ...
    if n_pass < MIN_SURVIVORS:
        lines.append("---")
        lines.append(
            f"**Phase C.1 failed: {n_pass} survivors < {MIN_SURVIVORS}. "
            "Parameter sensitivity did not rescue participation; return to Phase B.1 archetype design.**"
        )
        lines.append("")
    else:
        lines.append("---")
        lines.append(
            f"**Phase C.1 succeeded: {n_pass} survivors ≥ {MIN_SURVIVORS}. "
            "Proceed to downstream phases using these parameter-stable C1 variants.**"
        )
        lines.append("")
...
def _try_overlap_matrix(wfo_runs_root: Path, out_path: Path) -> bool:
    ...
    """
    wfo_runs_root = wfo_runs_root.resolve()
    var_dirs = [p for p in wfo_runs_root.iterdir() if p.is_dir()]
    if len(var_dirs) < 2:
        return False
    ...
    for var_dir in var_dirs:
        ...
        if run_dir is None:
            continue
        run_dirs.append((var_dir.name, run_dir))
    ...
    sets = {}
    for variant_id, run_dir in run_dirs:
        sets[variant_id] = _load_oos_trade_keys(run_dir)
        if not sets[variant_id] and run_dir.exists():
            for fold_dir in run_dir.iterdir():
...
def run_phaseC1_leaderboard(results_root: Path, write_overlap: bool = True) -> None:
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Phase C.1 — Build leaderboard and survivors from WFO v2 outputs.",
    )
    parser.add_argument(
        "--results-root",
        default=str(ROOT / "results" / "phaseC1"),
        help="Results root (e.g. results/phaseC1).",
    )
    parser.add_argument(
        "--no-overlap",
        action="store_true",
    ...
    run_phaseC1_leaderboard(Path(args.results_root), write_overlap=not args.no_overlap)
