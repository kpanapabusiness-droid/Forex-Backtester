...
def _format_trades_not_found_error(path: Path) -> str:
    ...
    parts.append("     python -m core.backtester -c config.yaml --results-dir <output_dir>")
    parts.append(
        "     (Or use scripts/phaseD3_run_sequence_backtest.py for Phase D sequence backtests.)"
    )
    parts.append(
        "  2. Or use --find-latest-trades to auto-select the most recent trades.csv under results/phaseD"
    )
...
def _load_trades(path: Path) -> pd.DataFrame:
    ...
    if not pair_col:
        raise ValueError(f"Cannot infer 'pair' column. Found columns: {list(df.columns)}")
    ...
    if not dir_col:
        raise ValueError(
            f"Cannot infer direction column. Tried: {DIRECTION_ALIASES}. Found: {list(df.columns)}"
        )
...
def _find_zone_events(labels: pd.DataFrame, zone_type: str) -> pd.DataFrame:
    ...
    for (pair, direction, run_id), grp in labels.groupby(["pair", "direction", "_run"]):
        ...
        dataset_split = (
            grp["dataset_split"].mode().iloc[0] if len(grp["dataset_split"].mode()) else "unknown"
        )
        mfe_at_start = (
            grp[grp["date"] == start_date]["mfe_40_r"].iloc[0] if zone_type == "C" else np.nan
        )
        events.append(
            {
                "pair": pair,
...
def _build_missed_mega_report(
    ...
    for pair, evs in by_pair.items():
        ...
        for ev in evs_sorted[:top_n]:
            ...
            if captured and first_entry_date:
                label_lookup = labels[
                    (labels["pair"] == ev["pair"]) & (labels["direction"] == ev["direction"])
                ]
                label_lookup = label_lookup[
                    label_lookup["date"].dt.strftime("%Y-%m-%d") == first_entry_date
                ]
...
def _build_discovery_vs_validation(
    ...
    for split in ("discovery", "validation"):
        ...
        m_b = capture_summary[
            (capture_summary["dataset_split"] == split) & (capture_summary["zone_type"] == "B")
        ]
        m_c = capture_summary[
            (capture_summary["dataset_split"] == split) & (capture_summary["zone_type"] == "C")
        ]
...
def _write_decision_memo(
    ...
    for split in ("discovery", "validation"):
        ...
        if rate >= 0.60:
            lines.append(
                "  => Entry captures most large trends; geometry/tail capture is bottleneck."
            )
        elif rate <= 0.30:
    ...
    if pd.notna(median_t6) and median_t6 <= 2:
        lines.append(
            "Timing lag note: median t6_at_entry for captured Zone C events is <= 2 bars — entry lag problem."
        )
    else:
        lines.append(
            "Timing lag note: median t6_at_entry for captured Zone C events > 2 bars — no entry lag problem."
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--capture-bars", type=int, default=5, help="Capture window in bars (default: 5)"
    )
    parser.add_argument(
        "--top-n", type=int, default=10, help="Top N mega events per pair (default: 10)"
    )
