"""Arc 8 v3.0.2 Step 3 — capturability.

Per L_PROTOCOL §2 Step 3 + intent doc §2:
  ...
  SL sweep per cluster: SL in {1.5, 2.0, 2.5, 3.0, 3.5, 4.0} x ATR
  ...
  via Amendment 5 four-gate rule).

Operates on Step 1 + Step 2 outputs — convention-agnostic (5ers_eet panels
already produced the pool / paths in Step 1).
"""
...
from scripts.l_arc_8_v3_0_2.shared import ARC_NAME, RESULTS_ROOT, SL_ATR_MULT_STEP1, SL_SWEEP
...
def _wrong_way_prevalence(mfe_r: pd.Series, mae_r: pd.Series) -> float:
    """ww_pp = P(MAE hits -1R before MFE hits +1R) — approximation."""
    n = len(mfe_r)
    if n == 0:
        return 0.0
...
def _cluster_archetype(metrics_df_per_cluster: pd.DataFrame, modal_tag: str) -> str:
    """Pick the canonical archetype taxonomy from cluster characteristics.

    Maps the Step 2 modal shape_tag (or Step 3 metric fallback) into one of
    {Stepwise climber, V-shape recovery, Bimodal, Monotonic up, Monotonic
    down, Choppy, Unclassified}. Drives Amendment 5 Gate 1 admission.
    """
...
def _build_summary_md(cap_df: pd.DataFrame, sweep_df: pd.DataFrame, metrics_step2: pd.DataFrame) -> str:
    lines = [
        f"# {ARC_NAME} — Step 3 Capturability Summary",
        "",
        f"_Generated: {datetime.now(timezone.utc).isoformat()}Z_",
        "",
        f"SL sweep: {list(SL_SWEEP)} x ATR.",
    ...
    lines += [
        ...
        "- **Gate 1 (Shape-required):** V-shape→A3; Stepwise→A4; Bimodal→A4; "
        "Monotonic up/down/Unclassified→(none)",
        "- **Gate 2 (Classifier-driven):** Step 4 mean OOS AUC ≥ 0.65 → add A2 + A6 "
        ...
        "## Methodology notes",
        "",
        "- Per-cluster MFE / MAE / final_R were rescaled from the Step-1 sim "
        f"(SL={SL_ATR_MULT_STEP1}xATR) by the closed-form rule in `_rescale_outcome_at_sl`.",
