...
def test_baseline_dual_use():
    from indicators.baseline_funcs import baseline_ema, ema
...
def test_exit_indicator_contract():
    ...
    df2 = exit_twiggs_money_flow(
        df.copy(), period=21, mode="threshold_cross", threshold=0.01, signal_col="my_exit"
    )
...
def test_backtester_dict():
    ...
    from backtester import load_config, run_backtest
...
def test_indicator_api_sweep():
    ...
    for role, (mod_name, prefix, sigcol) in roles.items():
        ...
        funcs = [
            (n, f) for n, f in inspect.getmembers(mod, inspect.isfunction) if n.startswith(prefix)
        ]
        for name, fn in funcs:
            try:
                ...
                vals = set(
                    pd.to_numeric(df[sigcol], errors="coerce").fillna(0).astype(int).unique()
                )
                if role == "exit":
