...
def load_metrics(run_path: Path):
    ...
    if not tp.exists():
        return None
    df = pd.read_csv(tp)
    wins = int((df.get("win")).sum()) if "win" in df else int((df["pnl"] > 0).sum())
    losses = int((df.get("loss")).sum()) if "loss" in df else int((df["pnl"] < 0).sum())
    scratches = int((df.get("scratch")).sum()) if "scratch" in df else int((df["pnl"] == 0).sum())
...
def main():
    ...
    if not args.a or not args.b:
        ...
        a, b = (
            runs_sorted[-2],
            runs_sorted[-1] if len(runs_sorted) >= 2 else (runs_sorted[0], runs_sorted[0]),
        )
    else:
        a = hist_dir / args.a
        b = hist_dir / args.b
    ...
    def fmt(m):
        return f"{m:.2f}" if isinstance(m, float) else str(m)
