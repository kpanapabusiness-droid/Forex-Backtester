"""Arc 2 redo — Step 2 path-shape clustering.

Per L_ARC_PROTOCOL.md v2.0 §§1, 6, 11:
  1. Compute four outcome-blind path-shape features per trade (still_open=1 only).
  2. K-means sweep K in {3..7} on StandardScaler-applied features.
  3. Apply §6 gate (silhouette >= 0.30, no cluster > 90%, all clusters >= 30).
  4. Select K (highest silhouette satisfying gate; tie -> smaller K).
  5. Label clusters per §11 centroid patterns; flag boundary clusters.
  6. Write outputs + STEP2_SUMMARY.md.
...
Usage:
  py scripts/arc_2_redo/step2_cluster.py -c configs/arc_2_redo/step2.yaml
"""
...
def _features_for_trade(
    ...
    Inputs are arrays restricted to still_open=1, ordered by bar_offset
...
def compute_path_features(paths_df: pd.DataFrame) -> pd.DataFrame:
    """Group trades_paths.csv by trade_id, compute features on still_open=1 subset."""
    held = paths_df[paths_df["still_open"] == 1].copy()
...
def write_histograms(features: pd.DataFrame, bins: int, out_path: Path) -> None:
    ...
    fig.suptitle("Arc 2 redo Step 2 — path-shape feature histograms (n=12,262)")
...
def select_k(
    results: Dict[int, KFitResult], gate: Dict[int, Tuple[bool, str]]
) -> Optional[int]:
    """Highest silhouette satisfying gate; ties broken by smaller K (parsimony)."""
    passing = [(k, results[k].silhouette) for k, (ok, _) in gate.items() if ok]
    if not passing:
        return None
    max_sil = max(s for _, s in passing)
    tied = sorted([k for k, s in passing if math.isclose(s, max_sil, abs_tol=1e-12)])
    return tied[0]
...
def write_summary(
    ...
    lines: List[str] = []
    lines.append("# Arc 2 redo — Step 2 path-shape clustering summary")
    lines.append("")
    lines.append("Protocol: `L_ARC_PROTOCOL.md` v2.0 §§1, 6, 11")
    lines.append("")
...
def _run_once(cfg: dict) -> Tuple[Dict[str, str], dict]:
    ...
    paths_df = pd.read_csv(
        paths_csv,
        usecols=["trade_id", "bar_offset", "close_r", "mfe_so_far_r", "still_open"],
    )
    ...
    chosen_k = select_k(results, gate)
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Arc 2 redo Step 2 — path-shape clustering.")
