...
"""Orchestrator: run the full L Arc 1 step 2 pipeline.
...
Phases:
  ...
  E: 15 shadow trade-sets (§5.11)
  F: cost stress (§5.12)
  G: random-entry baseline (§5.13)
  H: held-bar + forward context evolution (§5.14)
  I: phase doc + sanity checks + run manifest

Run with: py -m scripts.l_arc_1.step2.run_step2

Determinism check (separate, expensive): py -m scripts.l_arc_1.step2.run_determinism_check
"""
...
from scripts.l_arc_1.step2 import (
    phase_a_features,
    phase_a_lookahead,
    phase_b_marginals,
    phase_c_conditional,
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("L Arc 1 Step 2 — Descriptive Trade-Path Analysis")
    print("=" * 70)

    # Phase A at default H=240 first, to drive Phase D
    ...
    print("Outputs under results/l_arc_1/step2_descriptive/")
    print("=" * 70)
