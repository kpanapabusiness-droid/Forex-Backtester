"""Arc 11 — Step 3 capturability characterisation.

L_ARC_PROTOCOL.md v2.3 §§2, 7, 11, 17. SL sweep across
...
Usage:
    py scripts/l_arc_11/step3_capturability.py -c configs/l_arc_11/step3.yaml
"""
...
def write_histograms(unit_id: str, ev: _UnitEvalAtSL, trade_evals: List[_TradeEval], out_dir: Path) -> None:
    ...
    for arr, name, color in [(fwd, "fwd_mfe", "#4c72b0"), (fin, "final_r", "#dd8452")]:
        ...
        ax.set_title(
            f"Arc 11 Step 3 — {unit_id} {name} @ SL={ev.sl_atr_mult}×ATR "
...
def _run_once(cfg: dict) -> Tuple[Dict[str, str], dict]:
    ...
    for unit_id, unit_type, cids, tids, lp_centroid, rule, tlab in units:
        print(f"[l_arc_11 step3] evaluating {unit_id} (n={len(tids)}, peaks_centroid={lp_centroid:.2f})", file=sys.stderr)
...
def write_summary(
    ...
    lines: List[str] = []
    lines.append("# Arc 11 — Step 3 capturability characterisation summary")
    lines.append("")
    ...
    lines.append("- `configs/l_arc_11/step3.yaml`")
    lines.append("- `scripts/l_arc_11/step3_capturability.py`")
    lines.append("")
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Arc 11 Step 3 — capturability characterisation.")
    ap.add_argument("-c", "--config", type=Path, default=Path("configs/l_arc_11/step3.yaml"))
    ...
    print("[l_arc_11 step3] === RUN 1 ===", file=sys.stderr)
    hashes1, ctx = _run_once(cfg)
    hashes2 = None
    if bool(cfg["output"].get("determinism_check", True)):
        print("[l_arc_11 step3] === RUN 2 (determinism) ===", file=sys.stderr)
    ...
    print(f"[l_arc_11 step3] DONE. units_passing={n_pass}, det={det_gate}", file=sys.stderr)
