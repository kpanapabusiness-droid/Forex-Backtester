...
"""L Arc 2 Step 3 orchestrator — v1.1 amendment spec.
...
- Stratifications include arc-2 new axes (cum_logret_1h_24_bin etc.)

Output: results/l_arc_2/step3_extractability/
"""
...
from scripts.l_arc_2.step3 import _common as C
from scripts.l_arc_2.step3 import _data as D
from scripts.l_arc_2.step3 import _haircut as HC
from scripts.l_arc_2.step3 import _pca as PCA
from scripts.l_arc_2.step3 import _predictor as P
...
def main() -> int:
    ...
    log(
        f"  HDBSCAN: {n_hdbscan_clusters} non-noise clusters, "
        ...
        ari_rows.append(
            {
                ...
                "ari_kmeans_vs_K2_kmeans_baseline": CL.ari(hd[keep_mask], km_k2[keep_mask]),
                "ari_hierarchical_vs_K2_kmeans_baseline": CL.ari(
                    hd[keep_mask],
                    cluster_assignments[f"K{nearest_k}_hierarchical"].values[keep_mask],
                ),
            }
        )
            ...
            target_rows.append(
                {
                    ...
                    "target_cluster_size": best_n,
                    "target_cluster_size_fraction": best_frac,
        ...
        elig_rows.append(
            {
                ...
                "eligible_exit_or_delayed_entry_ge_5pct": bool(
                    f >= C.SIZE_FLOOR_EXIT_OR_DELAYED_ENTRY
                ),
            }
        )
    ...
    for drop_feat in base_feats:
        [f for f in base_feats if f != drop_feat]
        sub_df = df[keep_mask].copy()
        # Rebuild standardised matrix with one feature dropped
        rebuilt = sub_df[[f for f in C.CLUSTER_FEATURES_NUMERIC if f != drop_feat]].astype(float)
        ...
        mu = rebuilt.mean()
        sd_ = rebuilt.std(ddof=0).replace(0, 1.0)
        ...
        for k in C.K_VALUES if algo != "hdbscan" else [n_hdbscan_clusters]:
            ...
            for _, r in sub.iterrows():
                ...
                if algo == "kmeans":
                    ...
                    mean_ari = (
                        float(pert_k["ari_kmeans_vs_baseline"].mean())
                        if len(pert_k)
                        else float("nan")
                    )
                    ...
                    pert_k = pert_sub[pert_sub["K"] == k]
                    mean_ari = (
                        float(pert_k["ari_hierarchical_vs_baseline"].mean())
                        if len(pert_k)
                        else float("nan")
                    )
                else:
    ...
    calib_lines = [
        ...
        "and PHASE_L_ARC_2_OPEN.md §4 (Calibration check — not applicable).",
        "",
        "Arc 2 is a new signal with no known-prior carrier feature analogous",
    ...
    for t in C.HELD_BAR_TS:
        ...
        held_lookup = {
            f"{fname}_held_t{t}": held_at_t[fname].values for fname in C.HELD_CTX_NUMERIC
        }
    ...
    # =================================================================
    log("PHASE E: stratifications across axes (incl. arc-2 new axes)")
