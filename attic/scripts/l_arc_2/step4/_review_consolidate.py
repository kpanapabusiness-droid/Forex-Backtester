...
def verify_sources(manifest_path: Path) -> dict:
    ...
    for slug, fname in sources:
        ...
        if not match:
            all_ok = False
            print(
                f"FAIL: {rel} actual={actual[:12]} expected={(expected or 'NOT_FOUND')[:12]}",
                file=sys.stderr,
            )
        else:
...
def consolidate() -> pd.DataFrame:
            ...
            rows.append(
                {
                    ...
                    "frac_cluster_1_already_exited_at_t": float(
                        r["frac_cluster_1_already_exited_at_t"]
                    ),
    ...
    df = df.sort_values(["candidate_slug", "data_type", "fold_id_or_t"], kind="stable").reset_index(
        drop=True
    )
    return df
...
def validate(df: pd.DataFrame) -> None:
    ...
    for _, r in df.iterrows():
        ...
        assert not (eval_pop and tsel_pop), (
            f"row has both eval+tsel: {r['candidate_slug']} {r['data_type']} {r['fold_id_or_t']}"
        )
