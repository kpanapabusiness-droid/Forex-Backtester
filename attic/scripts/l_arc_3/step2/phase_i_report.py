...
def write_sanity_checks(extended: bool = False) -> Path:
    ...
    checks.append(
        {
            "id": 4,
            "check": "4 new arc-3 pre-signal context features present",
    ...
    checks.append(
        {
            ...
            "observed": "PASS"
            if det_pass
            else "PENDING (run scripts/l_arc_3/step2/run_determinism_check.py)",
    ...
    lines = [
        "L Arc 3 Step 2 — Sanity Checks (op spec §4 step 2 + task validation)",
        "=" * 70,
        "",
...
def write_run_manifest() -> Path:
    in_files = {
        "config (configs/wfo_l_arc3_verbatim.yaml)": REPO_ROOT
        / "configs"
        / "wfo_l_arc3_verbatim.yaml",
    ...
    lines = ["L Arc 3 Step 2 — Run Manifest", "=" * 60, "", "## Inputs (sha256)"]
...
def write_phase_doc(extended: bool) -> Path:
    ...
    md.append(
        f"| Input trade-set | `results/l_arc_3/step1_verbatim/trades_verbatim.csv` ({n_feat:,} rows) |"
    )
    md.append(
    ...
    md.append(
        "  - 1H signal TF, volatility_regime family, d1_atr_top_decile base, any sub-spec, h=120 horizon."
    )
    md.append("")
    ...
    md.append(
        det_text
        if det_text != "<missing>"
        else "(see determinism_check.txt — produced by scripts/l_arc_3/step2/run_determinism_check.py)"
    )
    md.append("```")
    md.append("")
    ...
    md.append("")
    md.append(
        f"- **{n_data_end} fold-7 data_end trades** preserved per step 1 handover. "
    ...
    md.append("")
    md.append(
        f"- `results/l_arc_3/step2_descriptive/signals_features.csv` "
    ...
    md.append(
        f"- `results/l_arc_3/step2_descriptive/trade_paths.csv` ({paths_count} rows; "
    ...
    md.append(
        f"~{100 * (f['exit_reason'] == 'data_end').mean():.2f}% data_end ({n_data_end} fold-7 tail trades)."
    )
    md.append(
    ...
    md.append("")
    md.append(
        "**Cross-arc carries from arc 1 and arc 2 (per arc-open §4) — discharged at step 2:**"
    )
    md.append("")
    md.append(
    ...
    md.append(
        f"- {n_data_end} fold-7 data_end trades carried through with `data_end_flag=True`. "
