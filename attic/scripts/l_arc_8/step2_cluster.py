...
L_ARC_PROTOCOL.md v2.3 stack (base v2.1.2 + v2.2 + v2.3 amendments); §§6, 11, 17 unchanged. Operates on Arc 8's §15a-compliant
...
Schema note carried from Step 1: Arc 8's mfe_so_far_r is running max of
...
Usage:
    py scripts/l_arc_8/step2_cluster.py -c configs/l_arc_8/step2.yaml
"""
...
def write_histograms(features: pd.DataFrame, bins: int, out_path: Path) -> None:
    ...
    fig.suptitle(f"Arc 8 Step 2 — path-shape feature histograms (n={len(features):,})")
...
def write_summary(
    ...
    lines: List[str] = []
    lines.append("# Arc 8 — Step 2 path-shape clustering summary")
    lines.append("")
    lines.append("Protocol: `L_ARC_PROTOCOL.md` v2.3 stack (base v2.1.2 + v2.2 + v2.3 amendments); §§6, 11, 17 unchanged")
    lines.append("")
    ...
    lines.append("- `configs/l_arc_8/step2.yaml`")
    lines.append("- `scripts/l_arc_8/step2_cluster.py`")
    lines.append("")
...
def _run_once(cfg: dict) -> Tuple[Dict[str, str], dict]:
    ...
    print("[l_arc_8 step2] loading trades_paths.csv (is_held column only — features use is_held=1)", file=sys.stderr)
    paths_df = pd.read_csv(
        paths_csv,
        usecols=["trade_id", "bar_offset", "close_r", "mfe_so_far_r", "is_held"],
    )
    print(f"[l_arc_8 step2] paths rows: {len(paths_df):,}", file=sys.stderr)

    print("[l_arc_8 step2] computing path-shape features", file=sys.stderr)
    features = compute_path_features(paths_df)
    print(f"[l_arc_8 step2] features computed for {len(features)} trades", file=sys.stderr)
    ...
    try:
        ...
        print(f"[l_arc_8 step2] histogram write skipped: {e}", file=sys.stderr)

    print("[l_arc_8 step2] k-means sweep", file=sys.stderr)
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Arc 8 Step 2 — path-shape clustering.")
    ap.add_argument("-c", "--config", type=Path, default=Path("configs/l_arc_8/step2.yaml"))
    ...
    chosen = ctx["chosen_k"]
    print(
        f"[l_arc_8 step2] DONE. chosen_K={chosen}, det={det_gate}, "
