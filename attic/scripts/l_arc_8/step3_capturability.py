"""Arc 8 — Step 3 capturability characterisation.

L_ARC_PROTOCOL.md v2.3 stack (base v2.1.2 + v2.2 + v2.3 amendments); §§2, 7, 11, 17 unchanged. v2.3 §4 adds pre_t_sl_atr_multiplier column to cluster_routing.csv. SL sweep across
...
Usage:
    py scripts/l_arc_8/step3_capturability.py -c configs/l_arc_8/step3.yaml
"""
...
def write_cluster_routing(out_path: Path, rows: List[Dict[str, Any]]) -> None:
    """cluster_routing.csv with v2.3 §4 pre_t_sl_atr_multiplier column.

    The pre_t_sl_atr_multiplier value records the per-archetype SL multiplier
    selected at Step 3 that Pipeline D1 will use at Step 5 WFO. Engine PR
    feat/open-24-pre-t-sl-per-archetype consumes this column. When the cluster
    dies at Step 3 (no SL passes), the column is empty.
    """
    cols = ["cluster_id", "tentative_label", "individual_pass", "aggregate_pass",
            "disposition", "final_archetype_label", "pre_t_sl_atr_multiplier",
            "notes"]
    with out_path.open("w", encoding="utf-8", newline="") as f:
        w = csv.writer(f, lineterminator="\n")
        w.writerow(cols)
        for r in rows:
            w.writerow([
                ...
                _fmt_g(r.get("pre_t_sl_atr_multiplier")),
                r["notes"],
            ])
...
def write_histograms(unit_id: str, ev: _UnitEvalAtSL, trade_evals: List[_TradeEval], out_dir: Path) -> None:
    ...
    for arr, name, color in [(fwd, "fwd_mfe", "#4c72b0"), (fin, "final_r", "#dd8452")]:
        ...
        ax.set_title(
            f"Arc 8 Step 3 — {unit_id} {name} @ SL={ev.sl_atr_mult}×ATR "
...
def _run_once(cfg: dict) -> Tuple[Dict[str, str], dict]:
    ...
    for unit_id, unit_type, cids, tids, lp_centroid, rule, tlab in units:
        print(f"[l_arc_8 step3] evaluating {unit_id} (n={len(tids)}, peaks_centroid={lp_centroid:.2f})", file=sys.stderr)
    ...
    for group in cfg["archetype_groups"]:
        ...
        for cid in cids:
            ...
            # v2.3 §4: pre_t_sl_atr_multiplier — per-archetype SL multiplier
            # that Pipeline D1 will use at Step 5 WFO. Sourced from the
            # individual-cluster selected SL when individual passes; else from
            # the aggregate selected SL when aggregate passes; else None.
            if indiv_pass:
                pre_t_sl = selected_sls.get(unit_id)
            elif agg_pass:
                pre_t_sl = selected_sls.get(agg_id)
            else:
                pre_t_sl = None
            routing_rows.append({
                "cluster_id": cid,
...
def write_summary(
    ...
    lines: List[str] = []
    lines.append("# Arc 8 — Step 3 capturability characterisation summary")
    lines.append("")
    lines.append("Protocol: `L_ARC_PROTOCOL.md` v2.3 stack (base v2.1.2 + v2.2 + v2.3 amendments); §§2, 7, 11, 17 unchanged. v2.3 §4 adds pre_t_sl_atr_multiplier column to cluster_routing.csv")
    lines.append("")
    if pass_rows:
    ...
    lines.append("- `configs/l_arc_8/step3.yaml`")
    lines.append("- `scripts/l_arc_8/step3_capturability.py`")
    lines.append("")
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Arc 8 Step 3 — capturability characterisation.")
    ap.add_argument("-c", "--config", type=Path, default=Path("configs/l_arc_8/step3.yaml"))
    ...
    print("[l_arc_8 step3] === RUN 1 ===", file=sys.stderr)
    hashes1, ctx = _run_once(cfg)
    hashes2 = None
    if bool(cfg["output"].get("determinism_check", True)):
        print("[l_arc_8 step3] === RUN 2 (determinism) ===", file=sys.stderr)
    ...
    print(f"[l_arc_8 step3] DONE. units_passing={n_pass}, det={det_gate}", file=sys.stderr)
