...
def diag_sharpe_summary(df: pd.DataFrame, trial_id: str, signal_tf: str) -> dict[str, float | int]:
    """Per-pair Sharpe diagnostic distribution for a given trial."""
    diag_prefix = "DIAG__" + trial_id[len("TRIAL__") :] + "__"
    rows = df[
        (df["stat_family"] == "l4_trial_diag")
...
def closure_disposition(top_n: pd.DataFrame, threshold: float, strict_gt: bool) -> str:
...
def emit_registry_md(top_n_data: list[dict], cfg: dict, l4_csv_path: Path) -> str:
    ...
    lines.append(
        f"- Top 5 spans **{len(family_counts)}** condition families "
    ...
    lines.append(
        f"- Signal-TF distribution in top 5: "
...
# ===========================================================================
def run(
    cfg: dict, out_md: Path | None = None, out_json: Path | None = None
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    print(
        f"[L5] n_proceeds_to_l6: {summary['n_proceeds_to_l6']} / {summary['n_top']}",
        file=sys.stderr,
    )
    return 0
