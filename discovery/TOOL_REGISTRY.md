...
| `observe_long_capture` | Step-(b) per-bar hypothetical-LONG OBSERVATION harness (re-rolled by arcs 1000–1006 + chat 3001's drift lens): honest +1R-before-SL CAPTURE (take-the-loss `reached_1r_before_sl`) + forward-drift in ATR, for every bar (or a `restrict` mask). Matches `build_arc_pool`'s entry/SL convention so the unconditional capture predicts the pool (verified: H4-majors 0.4877). Returns tidy `DataFrame[pair, signal_time, capture, fwd_drift_atr, atr]`; the arc joins its conditioning cols + groups. **CHARACTERIZATION ONLY — gross, NOT cost-aware, NOT a gate** (the gate is `MultiPairBacktester` via `ArcFoldRunner`); a pre-pool screen, never a verdict. | `discovery/tools/observe_long_capture.py` | `observe_long_capture(panel, sl_mult=2.0, hold=120, drift_bars=24)` → join conditioning → groupby | post-1007 refactor |
