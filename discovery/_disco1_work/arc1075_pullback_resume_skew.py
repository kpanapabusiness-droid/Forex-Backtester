"""arc 1075 — positive-skew PULLBACK-RESUME continuation on the HONEST ENGINE (last entry geometry).

Arc 1074 showed the Donchian-breakout-in-trend continuation entry is decisively negative on the engine
because it is ADVERSE-FIRST (buy the exact high -> whipsaw through the -1R stop on the majority). The
mechanistic complement (DISCOVERY_DIRECTION S1 take-the-loss-tax geometry; fbr is its reversion
instance): enter the continuation AFTER the pullback, on the RESUMPTION bar -> FAVORABLE-FIRST, so
take-the-loss falls on the noise trades not the trend trades. This driver spends the honest engine on
TrendPullbackResumeSignal (both directions) under the positive-skew RUNNER exits, judged by per-fold
MEAN ROI + median-per-fold + TAIL-REMOVED (+2R cap, drop-top-5%/topK) vs a fair same-side null. If THIS
geometry also fails the guard, positive-skew continuation is airtight-closed across all entry geometries.

CALLS canonical scoring (A1->MultiPairBacktester, FundedNext, risk 0.005); experiment side = entry +
tail/null ARITHMETIC only. IS 2010-2020; OOS touched ONLY if a cell passes G1+G2+G3. H4, 7 USD majors.
"""
...
from discovery.tools.trend_pullback_resume_signal import TrendPullbackResumeSignal
...
def _tail_stats(pnl_by_fold, sel, eval_ids, yr_of):
    ...
    return {"per_fold_roi": np.array(per_fold_roi), "per_fold_med_roi": np.array(per_fold_med_roi),
            "cap_roi": np.array(cap_roi), "drop5_roi": np.array(drop5_roi), "allr": allr,
...
def run_cell(panel, *, direction, pb, sf, ss, is_folds, yr_of, fn_cost, label):
    sig = TrendPullbackResumeSignal(direction=direction, sma_fast=sf, sma_slow=ss,
                                    pullback_lookback=pb).evaluate({"H4": panel})
    n_fires = sum(int(st.signal_mask.sum()) for st in sig.per_pair.values())
    runner = ArcFoldRunner(A1Architecture(), sig, {"H4": panel})
    scored, pnl_by_fold = {}, {}
    for lab, cfg in _grid(f"{direction}_pb{pb}").items():
    ...
    pos = int((roi > 0).sum()); afp = pos == len(roi)
    allr = ts["allr"]; mean_r, med_r = float(allr.mean()), float(np.median(allr))
    cap_mean = float(ts["cap_roi"].mean()); drop5_mean = float(ts["drop5_roi"].mean())
    ...
    if len(allr):
        ...
        largest = top1[0][0] / (RISK * SB)
    else:
        d1_mean = d3_mean = d5_mean = 0.0; largest = float("nan")

    print(f"\n{'='*92}\n{label}  | dir={direction} pb={pb} sma{sf}/{ss} | fires={n_fires} | "
    ...
    return {"sel": sel, "eval_ids": eval_ids, "mean_roi": mean_roi, "afp": afp,
            "g1": g1, "g2": g2, "g3": g3, "scored": scored, "pnl_by_fold": pnl_by_fold, "sig": sig}


def run_null(panel, cell, direction, pb, is_folds, yr_of, fn_cost):
    from discovery.tools.nested_exit_selection import freeze_best_over_folds
    if not len(cell["eval_ids"]):
        print("  NULL: skipped (no evaluable fires)")
        return
    frozen = freeze_best_over_folds(cell["scored"])
    cfg = _grid(f"{direction}_pb{pb}")[frozen]
    ...
    print(f"  NULL ({frozen}): real mean {real.mean()*100:+.4f}%/yr vs null mean {nullr.mean()*100:+.4f}%/yr "
          f"-> excess {(real.mean()-nullr.mean())*100:+.4f}pp "
          f"({'beats' if real.mean()>nullr.mean() else 'LOSES to'} null)")


def main():
    print("ARC 1075 — positive-skew PULLBACK-RESUME continuation on the HONEST ENGINE (favorable-first geometry)")
    ...
    cells = [
        ("PRIMARY long  pullback-resume sma20/100 pb10", "long", 10, 20, 100),
        ("PRIMARY short pullback-resume sma20/100 pb10", "short", 10, 20, 100),
        ("ROBUST  long  pullback-resume sma50/200 pb15", "long", 15, 50, 200),
        ("ROBUST  short pullback-resume sma50/200 pb15", "short", 15, 50, 200),
    ]
    results = []
    for label, d, pb, sf, ss in cells:
        cell = run_cell(panel, direction=d, pb=pb, sf=sf, ss=ss,
                        is_folds=is_folds, yr_of=yr_of, fn_cost=fn_cost, label=label)
        run_null(panel, cell, d, pb, is_folds, yr_of, fn_cost)
        results.append((label, cell))

    print(f"\n{'='*92}\nSUMMARY — IS guard decision (OOS spent ONLY for a cell passing G1+G2+G3)")
    any_pass = False
    for label, cell in results:
