"""arc 1076 — positive-skew trend-CONTINUATION on the HONEST ENGINE at the D1 timeframe.

Arcs 1074/1075/2081/2082 closed positive-skew continuation across every entry GEOMETRY, but ALL at
H4 — the single worst timeframe for trend-persistence-vs-noise. This driver varies the one axis those
arcs never touched: TIMEFRAME. It spends the honest engine on arc-1074's EXACT continuation entry
(TrendContinuationBreakoutSignal, both directions) at **D1** — the CTA timeframe where positive-skew
trend structurally lives and where a trailing runner can ride a multi-week move into a fat right tail.

Two priors point at the untested synthesis cell: arc 1002 triaged D1-long at -4.13% but NEVER under
the mandated mean+median+tail-removed lens / both dirs / §5f exit selection / full WFO; arc 1003 found
crosses TREND (gross +0.10R/trade) but H4 spreads sink the net. The synthesis neither tested: D1 ×
trendy crosses — D1 amortizes the wide cross spread over a long trend hold while keeping the stronger
cross trend. Run on two universes: 7 USD majors (controlled vs 1074/1002) AND the full 28-pair set.
...
BOUNDARY = "5ers_eet"
TF = "D1"  # THE new axis (vs H4 in arcs 1074/1075/2081/2082)
...
def run_cell(panel, *, direction, donch, sf, ss, is_folds, yr_of, fn_cost, label):
    sig = TrendContinuationBreakoutSignal(direction=direction, lookback=donch, sma_fast=sf,
                                          sma_slow=ss, primary_tf=TF).evaluate({TF: panel})
...
def main():
    print("ARC 1076 — positive-skew trend-CONTINUATION on the HONEST ENGINE at D1 (the untested TIMEFRAME axis)")
