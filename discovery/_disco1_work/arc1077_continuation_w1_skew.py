"""arc 1077 — positive-skew trend-CONTINUATION on the HONEST ENGINE at the W1 (weekly) timeframe.

Arcs 1074/1075/2081/2082/2083 closed positive-skew continuation across every entry GEOMETRY (all at
H4); arc 1076 closed the TIMEFRAME axis at D1. This driver runs the ONE remaining cell of that axis:
**W1 (weekly)** — the timeframe where the documented positive-skew trend premium (CTA / managed-futures
/ TSMOM) is strongest, and the literal completion of arc 1076's "untested TIMEFRAME axis" (which only
reached D1). arc 1076's own thesis ("a trailing runner can ride a multi-week move into a fat right
tail") is MAXIMALLY expressed at W1, where one winner can ride a multi-MONTH move = the fattest possible
right tail. If positive-skew continuation lives anywhere in-charter, W1 is its best chance; a clean KILL
makes the timeframe axis airtight (H4 + D1 + W1 all closed under the mandated lens).

W1-appropriate params (documented because): a 200-week SMA needs ~3.8yr warmup and is INFEASIBLE on the
11-yr IS window, so 1076's daily-Turtle SMA50/200 cannot be ported verbatim. Instead the canonical
WEEKLY long-term trend filter SMA10/40 (the 40-week MA ~ 200-day MA, Weinstein stage analysis; 10-week
~ 50-day) with Donchian{10,20}-week breakouts (the Turtle's 55-DAY ~ 11-week intermediate breakout
range). Same engine machinery / exits / guard / null as 1076 — only TF + trend params change.
...
BOUNDARY = "5ers_eet"
TF = "W1"  # THE new axis (vs H4 in 1074/1075/2081/2082/2083, D1 in 1076)

# Weekly-CTA trend params (200-week SMA infeasible on an 11-yr window — see module docstring)
SMA_FAST, SMA_SLOW = 10, 40
SPACING = 4          # ~1 month refractory between accepted weekly breakouts
NULL_WARMUP = 45     # >= SMA_SLOW so random entries sit after enough history
...
def run_cell(panel, *, direction, donch, is_folds, yr_of, fn_cost, label):
    sig = TrendContinuationBreakoutSignal(direction=direction, lookback=donch, sma_fast=SMA_FAST,
                                          sma_slow=SMA_SLOW, spacing_bars=SPACING,
                                          primary_tf=TF).evaluate({TF: panel})
    ...
    print(f"\n{'='*96}\n{label}  | dir={direction} donch={donch} sma{SMA_FAST}/{SMA_SLOW} | fires={n_fires} | "
...
def run_null(panel, cell, direction, donch, is_folds, yr_of, fn_cost):
    ...
    null_eval = build_null_signal_evaluation(cell["sig"], seed=42, warmup=NULL_WARMUP)
...
def run_universe(name, pairs):
    ...
    cells = [
        ("PRIMARY long  Donchian20 trend10/40", "long", 20),
        ("PRIMARY short Donchian20 trend10/40", "short", 20),
        ("ROBUST  long  Donchian10 trend10/40", "long", 10),
        ("ROBUST  short Donchian10 trend10/40", "short", 10),
    ]
    results = []
    for label, d, dn in cells:
        cell = run_cell(panel, direction=d, donch=dn,
...
def main():
    print("ARC 1077 — positive-skew trend-CONTINUATION on the HONEST ENGINE at W1 (the last untested TIMEFRAME cell)")
    print(f"HIST={HIST} CACHE={CACHE}\nTF={TF} sma{SMA_FAST}/{SMA_SLOW} runner exits={EXITS} SLs={SLS} "
          f"risk={RISK}; OOS untouched unless guard holds")
    ...
    for name, results in all_results:
        for label, d, dn, cell in results:
            ...
            print(f"  [{name}] {label:46s} G1={cell['g1']} G2={cell['g2']} G3={cell['g3']} AFP={cell['afp']} "
