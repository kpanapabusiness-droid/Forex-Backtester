...
The positive-skew continuation frontier was closed under the mandated mean+median+tail-removed lens
across entry geometry (1074/1075/1078/2081/2082/2083) and timeframe (1076 D1, 1077 W1) — but ALL of
those entries were TREND-FILTERED (a Donchian break INSIDE an established dual-SMA trend). The pure,
UNCONDITIONAL Donchian breakout — the canonical CTA / time-series-momentum entry, where the breakout
IS the trend signal (no prior-trend confirmation) — was run only by arc 2000: LONG-ONLY, MAJORS, H4,
CHEAP-KILLED at triage, with NO §5f exit selection, NO both-directions, NO D1, NO full WFO, and NO
formal median-per-fold + tail-removed guard. Arc 2000's decisive finding: the right tail is GENERIC
(a random/periodic long has the same tail; the breakout does not SELECT it).

This driver closes that exact cell airtight under the FULL mandated lens: unconditional Donchian
breakout, BOTH directions, at D1, on two universes (USD7 controlled + ALL28 trendy-cross steelman),
Donchian{20,55}, §5f-nested runner-exit selection, full 8-fold WFO, the formal G1/G2/G3 tail-luck
guard, and a fair same-side random-entry null. CALLS canonical scoring (ArcFoldRunner ->
A1Architecture -> MultiPairBacktester, FundedNext costs ON, risk 0.005); the experiment side is the
entry signal + tail-removal/null ARITHMETIC only. Never realizes P&L for a gate. IS 2010-2020 only;
OOS (2021+) touched ONLY if the IS guard (G1+G2+G3 + beats null) holds (printed decision).

Reuses arc 1076's exact driver structure verbatim; the ONLY change is the entry signal
(DonchianBreakoutSignal, unconditional) and dropping the SMA params from the grid/cells.
"""
...
from discovery.tools.donchian_breakout_signal import DonchianBreakoutSignal
...
def run_cell(panel, *, direction, donch, is_folds, yr_of, fn_cost, label):
    sig = DonchianBreakoutSignal(direction=direction, lookback=donch, primary_tf=TF).evaluate({TF: panel})
    ...
    print(f"\n{'='*96}\n{label}  | dir={direction} donch={donch} UNCONDITIONAL | fires={n_fires} | "
...
def run_universe(name, pairs):
    ...
    cells = [
        ("PRIMARY long  Donchian20 UNCONDITIONAL", "long", 20),
        ("PRIMARY short Donchian20 UNCONDITIONAL", "short", 20),
        ("ROBUST  long  Donchian55 UNCONDITIONAL (Turtle)", "long", 55),
        ("ROBUST  short Donchian55 UNCONDITIONAL (Turtle)", "short", 55),
    ]
    results = []
...
def main():
    print("ARC 1085 — positive-skew UNCONDITIONAL Donchian breakout on the HONEST ENGINE at D1")
    ...
    for name, results in all_results:
        for label, d, dn, cell in results:
            ...
            print(f"  [{name}] {label:48s} G1={cell['g1']} G2={cell['g2']} G3={cell['g3']} AFP={cell['afp']} "
