"""arc 2085 — positive-skew trend CONTINUATION on the WEEKLY timeframe (the last finer-grained TF cell).

Definitive final closer of the TIMEFRAME axis. arc 2084 found D1 bleeds ~8x less than H4 (timeframe is a
geometry lever) but never crosses zero (median trade still -1R = not a sign lever). W1 is the only
finer-grained cell left; the D1->H4 gradient + W1 thinness predict the same closure with worse fold
resolution. Same harness as 2084, WEEKLY-appropriate params (sma 10/40 ~= 50/200-day; Donchian {10,26}).
A clean KILL (or a pool-floor/fold-resolution KILL) closes the timeframe axis completely.

(original 2084 header retained below)
...
def run_null(panel, cell, direction, donch, is_folds, yr_of, fn_cost):
    ...
    null_eval = build_null_signal_evaluation(cell["sig"], seed=42, warmup=45)
...
def main():
    print("ARC 2085 — positive-skew trend CONTINUATION on the WEEKLY timeframe (final timeframe-axis closer)")
    ...
    # WEEKLY-appropriate params: sma 10/40 (~50/200-day), Donchian {10,26} weeks (~50/130-day)
    cells = [
        ("PRIMARY long  Donchian10 trend10/40", "long", 10, 10, 40),
        ("PRIMARY short Donchian10 trend10/40", "short", 10, 10, 40),
        ("SLOW    long  Donchian26 trend10/40", "long", 26, 10, 40),
        ("SLOW    short Donchian26 trend10/40", "short", 26, 10, 40),
    ]
    results = []
