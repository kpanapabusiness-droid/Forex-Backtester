...
| **(this PR)** | §6.3 spread P&L decomposition diagnostic + ledger schema extension (bid/ask + sl_price) | New §6.3 capability: `core/step_6/spread_pnl_decomposition.py` decomposes per-trade R into signal vs spread tax, projects under inflation scenarios `(1.25, 1.50, 2.00, 3.00)`, classifies fragility (`robust`/`tolerant`/`marginal`/`fragile`), reports verdict-flip factor. Info-severity check — never modifies verdict. Required ledger extension: `entry_bid`, `entry_ask`, `exit_bid`, `exit_ask` on `ClosedTrade` + `ArcPool.trades`; `sl_price` on `ClosedTrade` (Position already had it). KH-24 anchor verified byte-identical pre/post (7-fold Mode A). Grandfathered: pre-PR closures skip the diagnostic gracefully. |
| **#2XX** | Amendment 3.1 — `r_max` as deployment cap | `core/wfo/amended_gates.py` gate logic update + tracker schema additions + L_PROTOCOL §3 sub-amendment. Removes ceiling-overshoot branch of `step5_not_scalable`. Adds `r_safe_capped_at_rmax` / `r_hard_capped_at_rmax` tracker fields. |
