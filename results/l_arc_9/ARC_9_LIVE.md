...
- Disposition: **STEP_4_KILL — REAFFIRMED 2026-05-19 after causal patch.** The intermediate "PASS-DEPLOYABLE-PENDING-AMENDMENT" picture from experiments 4-7 was invalidated by a producer-level lookahead leak in two D1 swing features (`d1_bars_since_swing_low`, `d1_bars_since_swing_high`) discovered by external review post-audit. Patched LightGBM AUC drops 0.7508 → 0.5190 (−0.23), below §8 0.65 gate. Step 5 LGBM E Candidate A/B economics, scaled-risk 1.0% recommendation all invalidated. **Cohort is still real** (Step 3 / Step 5 oracle unaffected — path-shape features only, no D1). **Held-open lifecycle terminates pending fresh research approach.**
...
| Candidate A scaled-risk WFO (`results/l_arc_9/experiments/scaled_risk/`) ⚠ INVALIDATED 2026-05-19 | Re-account the fixed 236-trade admit set at 5 per-trade risk levels {0.5%, 1.0%, 1.5%, 2.0%, 2.5%} with intraday compounded worst-day DD measurement. Establishes the **measured** deployment risk, replacing the projection-based 2% in the candidate spec. | **Recommended deployment risk: 1.0% per trade** (half the projection). At 1.0%: worst-fold max DD 2.52% / **worst-day DD 2.01%** / worst-fold ann ROI +20.07% / full-data ann ROI +41.45% / all F2-F7 positive. **Binding constraint is worst-day DD, NOT worst-fold max DD** — a metric the projection didn't measure. **Single day (2024-08-06, 2 same-day SL trades on GBP_CHF + NZD_USD, net -2.02R) drives the worst-day at every risk level.** DD scales exactly linearly with risk (DD ratio observed / linear ≈ 1.00); ROI scales supra-linearly via compounding (2.17× at 2× risk, 7.04× at 5× risk). **⚠ INVALIDATED 2026-05-19 by causal patch dispatch: the underlying Candidate A admit set was built on a leaked classifier (AUC 0.7508 fake → patched 0.5190). Per-trade-risk recommendation, worst-day cluster identification, and scaling sanity all derived from invalidated admissions and DO NOT apply.** The methodology lesson — worst-day DD at intraday compounded resolution as a standard Step 5 output — survives independently of the leak and applies to any future correctly-derived candidate. |
| Causal patch + re-run (`results/l_arc_9/experiments/causal_patch/`) + analyst-override forced WFO (`step5_wfo_forced/`) | Corrective dispatch triggered by external review finding producer-level lookahead in two D1 swing features. Original 8/8 GREEN audit checked join-level causality but missed producer-level scope. Patch the two leaked features (Option A confirmed-swing with 10-day lag), audit ALL 8 D1 features at producer level (NEW audit dimension), re-run Pipeline E retry + conditionally Step 5 + scaled-risk. Phases 4-5 skipped per dispatch hard rule (patched AUC < gate); subsequently re-run under analyst override for diagnostic completeness. | **STEP_4_KILL_AFTER_PATCH.** Patched LGBM mean CV AUC drops from **0.7508 → 0.5190** (delta **−0.2318**), well below §8 gate 0.65. Phases 4 (Step 5 WFO) and 5 (scaled-risk) correctly skipped per dispatch hard rule. Per-fold drop consistent (−0.16 to −0.28 across all 5 TSS folds) — not a single-fold artifact. Cells comparison: baseline (16 features) byte-identical to original; expanded (28 features) drops −0.22 RF / −0.23 LGBM. **Feature expansion lift collapses from +0.226 to −0.0056 (LGBM).** Producer-level causal audit on all 7 D1 features in the D1 frame: 700/700 (sample × feature) match — patched code is causally clean by construction; other 6 features (RSI, ATR, Kijun, trend state, 20d pos, ret 5d) were already clean (one-sided recursive smoothers / rolling windows / scalar comparisons). What survives: cohort identification (Step 3, cluster_0 fwd_mfe_p50 6.18R / final_r_mean +4.423R), Step 5 oracle (cluster 0 only, +60% ann ROI), Step 5 raw baseline (FAIL all gates), join-level lookahead audit on its checked dimensions. What collapses: Pipeline E retry AUC 0.7508 (fake), Step 5 LGBM E Candidate A/B economics, scaled-risk 1.0% recommendation, "features over classifiers" methodology lesson. **v2.x amendment impact:** §8 D1 feature-budget expansion case COLLAPSES (Arc 9 contributes nothing); §3 grid relaxation WEAKENED (Arc 9's pattern was on a fake classifier; Arc 7 D1 remains as load-bearing evidence). **NEW v2.x amendment evidence:** producer-level causal scope must be a standard audit dimension going forward — the original GREEN audit's checked dimensions (merge_asof + days_lag + end-to-end probability reproduction) were insufficient. Held-open lifecycle terminates; analyst writes consolidated closure reverting Arc 9 to STEP_4_KILL with incident-corrected v2.x evidence. **Analyst-override forced WFO addendum (`step5_wfo_forced/`):** for diagnostic completeness the dispatch-skipped Phase 4 + 5 were re-run with the patched classifier. Both candidates fail decisively — Candidate A worst-fold ann ROI **−6.61%** (F6) / full-data ann ROI **−0.13%** / full-data DD **13.38%**; Candidate B worst-fold **−19.37%** (F6) / full **+0.08%** / **DD 22.65%** (close to raw-baseline floor 62.99%, well past 5ers 10% hard limit). Both fail restricted §10 pass-deployable on F2-F7. Scaled-risk skipped for both. Confirms the §8 AUC gate was correctly tight: had the gate been bypassed, downstream §10 economics would have rejected the same candidates with direct evidence. The original Candidate A/B economics (+9.63% / +20.68% worst-fold) were entirely leak-driven. |
