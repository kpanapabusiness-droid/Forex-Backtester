"""Arc 2 redo2 — Step 1 plumbing: build trade pool + trade paths.
...
Forked from scripts/arc_2_redo/step1_build_pool.py with two surgical fixes for
v2.1.1 §5 / §17 schema compliance:

  1. Path-column rename: `still_open` -> `is_held` (identical semantics;
     1 = bar is held / PnL-bearing, 0 = forward observation / no PnL impact).
  2. Forward-observation R-fields (`close_r`, `mfe_so_far_r`, `mae_so_far_r`)
     for bar_offset > exit_offset are computed from real-market price data
     instead of being frozen at the exit-bar values. Required so §7 SL sweep
     (v2.1.1) can re-impose hypothetical SLs on the R-frame directly. Matches
     the semantics of `scripts/phase_kgl_v2_4h_wfo.py::_flatten_bar_path_for_trade`
     v1.3 forward-window extension.

  is_held=0 R-fields are forward observation for §7 SL sweep only — never use
  for PnL or trade execution. `trades_all.csv` records only actual trade
  outcomes; forward bars do not affect equity, exits, or any deployed behaviour.

Outputs (under cfg.output.results_dir, default results/l_arc_2_redo2/step1/):
...
Usage:
  py scripts/arc_2_redo2/step1_build_pool.py -c configs/arc_2_redo2/step1.yaml
"""
...
def _build_pair_trades(
    ...
    for sig_idx in sig_positions:
        ...
        # Post-exit bars (is_held=0): close_r/mfe/mae are computed from
        # real-market OHLC at the post-exit bar via the same R-normalisation
        # formula (close - entry_price) / sl_distance_price. mfe_so_far /
        # mae_so_far continue to evolve through forward bars, mirroring
        # scripts/phase_kgl_v2_4h_wfo.py::_flatten_bar_path_for_trade. These
        # forward-observation R-fields exist solely for §7 SL sweep — they
        # have NO PnL impact and MUST NOT feed into trades_all.csv or any
        # downstream trade-decision logic.
        ...
        for off in range(0, window + 1):
            ...
            if off < exit_offset:
                ...
                # Exit bar — close_r is realized R (with spread).
                close_r = final_r
                is_held = 1
                exit_event = exit_reason
            else:
                # Post-exit — forward observation. Real-market R-normalised
                # close (no spread; observation only, no PnL).
                close_r = (c - entry_price) / sl_distance_price
                is_held = 0
                exit_event = "none"
