...
from scripts.l_arc_1.step2._io import (
    FORWARD_HORIZON_BARS_DEFAULT, H_GRID, PAIRS, STEP2_DIR, compute_signal_mask,
...
def per_trade_forward_path(open_arr: np.ndarray, high_arr: np.ndarray,
...
def compute_path_aggregates(fhigh: np.ndarray, flow: np.ndarray,
                            ...
                            atr: float, H: int) -> Dict[str, float]:
    out: Dict[str, float] = {}
    for h in H_GRID:
        if h > H:
    ...
    out["fwd_time_to_peak_mfe"] = float(time_to_peak_mfe) if np.isfinite(time_to_peak_mfe) else np.nan
    out["fwd_time_to_trough_mae"] = float(time_to_trough_mae) if np.isfinite(time_to_trough_mae) else np.nan
...
def run_phase_a(H: int = FORWARD_HORIZON_BARS_DEFAULT) -> dict:
    ...
    for pair in PAIRS:
        ...
        pair_logret[pair] = lr
        thr = (
            pd.Series(np.abs(lr)).rolling(100, min_periods=100)
            .quantile(0.90, interpolation="linear").shift(1).to_numpy()
        )
        pair_thr[pair] = thr
    ...
    tr_net_r = trades["net_r"].astype(float).to_numpy() if "net_r" in trades.columns else trades["R"].astype(float).to_numpy()
    tr_gross_r = trades["gross_r"].astype(float).to_numpy() if "gross_r" in trades.columns else np.full(n_trades, np.nan)
    tr_spread_R = trades["spread_cost_R"].astype(float).to_numpy() if "spread_cost_R" in trades.columns else np.full(n_trades, np.nan)
    ...
    pw.writerow(["trade_id", "t", "fwd_logret_step", "fwd_logret_cum", "fwd_mfe_atr", "fwd_mae_atr"])
    ...
    for k in range(n_trades):
        ...
        if sig_idx is None:
            ...
            sig_idx = int(np.searchsorted(
                pair_data[pair]["time"].astype("datetime64[ns]").astype("int64").values, tr_sig_ts[k],
            ))
        entry_idx = sig_idx + 1
        ...
        trigger_excess = (sig_abs_lr - thr) if (np.isfinite(sig_abs_lr) and np.isfinite(thr)) else np.nan
        trigger_ratio = (sig_abs_lr / thr) if (np.isfinite(sig_abs_lr) and np.isfinite(thr) and thr > 0) else np.nan
        ...
        for t in range(1, H + 1):
            pw.writerow([
                ...
                f"{(fhigh[t-1] / atr_at_sig):.6g}" if (np.isfinite(fhigh[t-1]) and atr_at_sig > 0) else "",
                f"{(flow[t-1] / atr_at_sig):.6g}" if (np.isfinite(flow[t-1]) and atr_at_sig > 0) else "",
            ])
        ...
        mae_held_atr = abs(mae_R) * sl_atr_mult  # convert engine's signed mae_R to positive magnitude
