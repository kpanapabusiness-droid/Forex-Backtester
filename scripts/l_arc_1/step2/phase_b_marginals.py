...
from scripts.l_arc_1.step2._distributions import (
    describe_distribution, histogram_csv, write_distribution,
...
def _emit(values: pd.Series, subdir: str, metric: str, *,
          degenerate: bool = False, reason: str = "",
          features: pd.DataFrame | None = None,
          fold_col: str = "fold_id") -> None:
    out_dir = DIST_DIR / subdir
    out_dir.mkdir(parents=True, exist_ok=True)
    write_distribution(
        values.tolist(), out_dir / f"{metric}.csv",
        metric_name=metric, degenerate=degenerate,
...
def run_phase_b() -> None:
    ...
    _emit(f["mfe_held_atr"], "marginals", "mfe_held_atr", features=f,
          degenerate=True, reason="h=1 held window is 1 bar; identical to fwd_mfe_h1_atr")
    _emit(f["mae_held_atr"], "marginals", "mae_held_atr", features=f,
          degenerate=True, reason="h=1 held window is 1 bar; identical to fwd_mae_h1_atr")
    _emit(f["bars_held"], "marginals", "bars_held", features=f,
          degenerate=True, reason="time-exit h=1; all trades held 1 bar (except sl_hit which is intrabar)")
    ...
    _emit(f["peak_to_final_r_ratio"], "marginals", "peak_to_final_r_ratio", features=f,
          degenerate=True, reason="peak/final ratio over a 1-bar held window")
    _emit(f["mfe_to_mae_ratio_held"], "marginals", "mfe_to_mae_ratio_held", features=f,
          degenerate=True, reason="ratio over a 1-bar held window")
    ...
    _emit(diff_held, "sequence", "time_to_peak_mfe_minus_trough_mae_held",
          features=f, degenerate=True,
    ...
    _emit(diff_fwd, "sequence", "time_to_peak_mfe_minus_trough_mae_fwd",
          features=f)
    ...
    for h in (24, 120):
        ...
        for cls in sorted(f[seq_col].unique()):
            ...
            rows.append({
                ...
                "median_fwd_mfe_h24_atr": float(sub.get(f"fwd_mfe_h24_atr", pd.Series([np.nan])).median()),
                "median_fwd_mae_h24_atr": float(sub.get(f"fwd_mae_h24_atr", pd.Series([np.nan])).median()),
                "median_fwd_mfe_h120_atr": float(sub.get(f"fwd_mfe_h120_atr", pd.Series([np.nan])).median()),
                "median_fwd_mae_h120_atr": float(sub.get(f"fwd_mae_h120_atr", pd.Series([np.nan])).median()),
                "exit_reason_sl_hit_pct": float((sub["exit_reason"] == "stop_loss").mean() * 100.0),
            })
    ...
    for col in ("fwd_oscillation_count", "fwd_monotonicity_ratio",
                "fwd_max_consecutive_with", "fwd_max_consecutive_against",
                "fwd_acf1_returns", "fwd_time_to_peak_mfe",
    ...
    _emit(time_from_peak_to_exit_fwd, "complexity",
          "time_from_peak_mfe_to_window_end_fwd", features=f)
    ...
    _emit(r_given_back, "complexity",
          "r_given_back_from_peak_fwd_atr_units", features=f)
    # held-window degenerate placeholders
    out_dir = DIST_DIR / "complexity"
    pd.DataFrame([{"metric": "held_window_complexity",
                   "note": "degenerate; held window is 1 bar — see fwd_* columns above"}]).to_csv(
        out_dir / "held_window_complexity_degenerate.csv", index=False, lineterminator="\n"
    )
    ...
    eb_rows = [{"t": 1, "corr_cumR_t_to_finalR": 1.0,
    ...
    with eb_path.open("w", encoding="utf-8", newline="") as fh:
        fh.write("# degenerate_by_construction: true | reason: h=1 trades have final R = cum R at t=1\n")
