...
def write_sanity_checks() -> Path:
    ...
    checks.append(
        {
            "id": 1,
            "check": "signals_features.csv row count = trades_verbatim.csv row count",
    ...
    checks.append(
        {
            "id": 2,
            "check": f"trade_paths.csv row count = 45,673 × H ({H} after Phase D extension)",
    ...
    checks.append(
        {
            "id": 3,
            "check": "All op spec §5 angle subfolders present under step2_descriptive/",
    ...
    if csw_present:
        ...
        observed = (
            f"present; nonzero={csw_nonzero}; pool median="
    ...
    checks.append(
        {
            "id": 4,
            "check": "concurrent_signals_within_3h column present and non-trivial",
            "expected": "present and non-trivial",
            "observed": observed,
            "status": csw_status,
        }
    )
    ...
    checks.append(
        {
            "id": 5,
            "check": "Lookahead-invariant features test (100-sample perturbation)",
            "expected": "PASS",
    ...
    checks.append(
        {
            "id": 7,
            "check": "Forward-horizon stability check produced; outcome documented",
    ...
    checks.append(
        {
            "id": 8,
            "check": "Random-entry baseline produced; visible difference from verbatim documented",
    ...
    checks.append(
        {
            "id": 9,
            "check": "Sample-size discipline (n<30 flagged in conditional breakdowns)",
            "expected": "applied",
    ...
    checks.append(
        {
            "id": 10,
            "check": "Distribution shape per op spec §11.1 (mean/std/skew/kurt/min/p1.../p99/max)",
            "expected": "full shape",
    ...
    checks.append(
        {
            "id": 11,
            "check": "2D distributions reported as heatmaps, not correlations (op spec §11.2)",
    ...
    checks.append(
        {
            "id": 12,
            "check": "Per-fold breakdowns for every aggregated metric (op spec §11.3)",
            "expected": ">=20",
    ...
    checks.append(
        {
            ...
            "observed": "PASS"
            if det_pass
            else ("FAIL/MISSING — run scripts/l_arc_1/step2/run_determinism_check.py"),
    ...
    checks.append(
        {
            "id": 14,
            "check": "Phase doc contains zero action-shaped or recommendation-shaped language (op spec §11.5)",
    ...
    lines = [
        "L Arc 1 Step 2 — Sanity Checks (per prompt validation checklist)",
        "=" * 70,
        "",
...
def write_run_manifest() -> Path:
    ...
    # Inputs
    in_files = {
        "config (configs/wfo_l_arc1_verbatim.yaml)": REPO_ROOT
        / "configs"
        / "wfo_l_arc1_verbatim.yaml",
...
def write_phase_doc() -> Path:
    ...
    p50_fwd_mfe_h480 = (
        _safe_median(f["fwd_mfe_h480_atr"]) if "fwd_mfe_h480_atr" in f.columns else float("nan")
    )
    ...
    # Random baseline
    pd.read_csv(STEP2_DIR / "random_baseline" / "comparison.csv")
    ...
    md_lines.append(
        "| Input trade-set | `results/l_arc_1/step1_verbatim/trades_verbatim.csv` (45,673 rows) |"
    )
    md_lines.append(
        "| Forward horizon | H = 480 (extended from default 240 per Phase D — see §7) |"
    )
    ...
    md_lines.append("")
    md_lines.append(
        f"- `signals_features.csv`: **{n_feat:,} rows** (one per taken trade), {n_cols} columns."
    )
    md_lines.append(
        "- `trade_paths.csv`: **{:,} rows** ({:,} trades × {} forward bars).".format(
    ...
    md_lines.append(
        "  - **trade-level outcome**: net_r, gross_r, spread_cost_R, mfe_R, "
    ...
    md_lines.append(
        "  whose entry bar is fewer than H bars before the dataset end. "
    ...
    md_lines.append(
        "| `mfe_sequence_class_held` | All rows = `simultaneous_bar` | `mfe_sequence_class_fwd_h{24,120}` on the forward path |"
    )
    md_lines.append(
        "| Held-bar evolution at t∈{3,5,10,20} | Vacuous | `forward_context_evolution/` t∈{1,3,5,10,20} |"
    )
    md_lines.append(
        "| Early-bar predictivity at t=1 | cum_R(t=1) ≡ final R; corr trivially 1.0 | n/a |"
    )
    md_lines.append("| Survival at t≥2 | Always 0 (trades closed) | n/a |")
    md_lines.append(
        "| Path complexity (§5.5) on held window | Trivial on 1 bar | Recast on forward path (H=480) |"
    )
    md_lines.append("")
    ...
    md_lines.append(
        "  max_consecutive_with/against, acf1_returns, time_to_peak_mfe, "
    ...
    md_lines.append(
        "- **Cross-pair correlation regime (§5.14)** — approximated as `cross_pair_dispersion_proxy`"
    )
    ...
    md_lines.append("")
    md_lines.append(
        "- **net_r**: pool p50 = {:.4f}, mean = {:.5f}, std = {:.4f}, p95 = {:.3f}, p5 = {:.3f}".format(
            p50_net_r,
            _safe_mean(f["net_r"]),
            float(f["net_r"].std()),
            _safe_quantile(f["net_r"], 0.95),
            _safe_quantile(f["net_r"], 0.05),
        )
    )
    md_lines.append(
        "- **fwd_mfe_h24_atr**: p50 = {:.3f}, p95 = {:.3f}; **fwd_mae_h24_atr**: p50 = {:.3f}".format(
            p50_fwd_mfe_h24, p95_fwd_mfe_h24, p50_fwd_mae_h24
        )
    )
    md_lines.append(
        "- **fwd_mfe_h240_atr**: p50 = {:.3f}; **fwd_mfe_h480_atr**: p50 = {:.3f}".format(
            p50_fwd_mfe_h240, p50_fwd_mfe_h480
        )
    )
    md_lines.append(
        "- **mfe_held_atr** (= fwd_mfe_h1_atr): mean = {:.4f}, p95 = {:.3f}".format(
            _safe_mean(f["mfe_held_atr"]), _safe_quantile(f["mfe_held_atr"], 0.95)
        )
    )
    md_lines.append(
        "- **Exit-reason mix**: time_exit = {:.2f}%, stop_loss = {:.2f}%".format(
    ...
    md_lines.append("")
    md_lines.append(
        f"Pool mean: {race_mean:.2f} bars. Capping convention: H+1 = 481 when never reached."
    )
    ...
    md_lines.append("")
    md_lines.append(
        "(Full distribution shape in `distributions/forward/race_bars_plus1_minus_minus1.csv`.)"
    )
    md_lines.append("")
    ...
    md_lines.append(
        sl_curve[
            ["sl_atr_mult", "n", "mean_net_r", "median_net_r", "win_pct", "frac_sl_hit"]
        ].to_string(index=False)
    )
    md_lines.append("")
    ...
    md_lines.append(
        "3-position rolling sum over the unified-timeline of fires across all 28 pairs."
    )
    md_lines.append(
        "Distribution shape in `distributions/marginals/concurrent_signals_within_3h.csv`."
    )
    md_lines.append(
        "Other §5.15 features (concurrent_signals_same_bar, currency_basket_3h_USD/EUR/"
    )
    ...
    md_lines.append(
        det_text
        if det_text != "<missing>"
        else "(see determinism_check.txt — run scripts/l_arc_1/step2/run_determinism_check.py)"
    )
    md_lines.append("```")
    md_lines.append("")
    ...
    md_lines.append(
        f"  ({n_feat:,} rows × {n_cols} columns; trade_id-keyed; all op spec §5.16 + L6.0 §14.3 features + §5.15 cross-pair)."
    )
    md_lines.append("- `results/l_arc_1/step2_descriptive/trade_paths.csv`")
    md_lines.append(
        f"  ({n_feat * 480:,} rows; (trade_id, t in [1..480])-keyed; per-bar fwd_logret_step, fwd_logret_cum, fwd_mfe_atr, fwd_mae_atr)."
    )
    md_lines.append("")
    md_lines.append("**Descriptive summary of trade-set shape (starting context for step 3):**")
    md_lines.append("")
    md_lines.append("The trade-set is 45,673 long trades across 28 pairs over 7 anchored")
    md_lines.append("expanding folds (Oct 2020 – Jan 2026), held one bar each at h=1 with a")
    md_lines.append(
        f"2 × ATR(14) hard stop. The pool median net R is {p50_net_r:.4f}; "
    ...
    md_lines.append(
        f"p50 fwd_mae = {p50_fwd_mae_h24:.2f} ATR; race-condition p50 = {race_pcts[50]:.1f} bars"
    )
    md_lines.append("(slight MAE-first skew at the median). Random-entry baseline on matched")
    ...
    md_lines.append(
        f"`concurrent_signals_within_3h` has pool median {csw_summary['p50']:.0f}, p95 "
