...
def fit_logreg(X: np.ndarray, y: np.ndarray, seed: int) -> tuple[np.ndarray, np.ndarray, float, dict]:
...
def run_model_perfold(model: str, X: np.ndarray, y: np.ndarray,
    ...
    for f in folds:
        ...
        if model == "logreg":
            ...
            clf = RandomForestClassifier(n_estimators=200, max_depth=5,
                                         random_state=seed + int(f), n_jobs=-1)
        elif model == "gb":
            clf = HistGradientBoostingClassifier(max_iter=200, max_depth=3,
                                                 learning_rate=0.05,
    ...
    for f in folds:
        ...
        if model == "logreg":
            ...
            clf = RandomForestClassifier(n_estimators=200, max_depth=5,
                                         random_state=seed + 7 + int(f), n_jobs=-1)
        elif model == "gb":
            clf = HistGradientBoostingClassifier(max_iter=200, max_depth=3,
                                                 learning_rate=0.05,
                                                 random_state=seed + 7 + int(f))
        clf.fit(X[tr_mask], y[tr_mask])
