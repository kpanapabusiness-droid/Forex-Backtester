...
def main() -> int:
    ...
    log(f"  added 3 new features: fwd_realized_range_atr, "
        f"fwd_fraction_time_above_entry, fwd_max_consecutive_directional_bars")
    ...
    log(f"  PC1+PC2+PC3 cumulative variance: {pc_summary['cumulative_variance_ratio'].iloc[-1]:.3f}")
    ...
    if hdbscan_skipped_no_clusters:
        log(f"  HDBSCAN found < 2 non-noise clusters at min_cluster_size=5% pool; "
            f"skipping HDBSCAN from target selection + Phase C/D (Amendment 3 reportorial)")
    log(f"  HDBSCAN: {n_hdbscan_clusters} clusters, {noise_count} noise points ({100*noise_count/n:.1f}%)")
    ...
    for k in C.K_VALUES:
        ari_rows.append({
            ...
            "ari_kmeans_vs_K2_kmeans_baseline": CL.ari(km_k2, cluster_assignments[f"K{k}_kmeans"].values),
            "ari_hierarchical_vs_K2_kmeans_baseline": CL.ari(km_k2, cluster_assignments[f"K{k}_hierarchical"].values),
        })
    ...
    ari_rows.append({
        ...
        "ari_kmeans_vs_hierarchical_sameK": CL.ari(hd, cluster_assignments[f"K{nearest_k}_kmeans"].values),
        "ari_kmeans_vs_K2_kmeans_baseline": CL.ari(hd, km_k2),
        "ari_hierarchical_vs_K2_kmeans_baseline": CL.ari(hd, cluster_assignments[f"K{nearest_k}_hierarchical"].values),
    })
        ...
        for k in C.K_VALUES:
            ...
            for cid in sorted(set(lab.tolist())):
                ...
                for m in dist_metrics:
                    ...
                    dist_rows.append({"algo": algo, "K": k, "cluster_id": int(cid), "metric": m, **stats})
                dist_rows.append({
        ...
        for k in C.K_VALUES:
            ...
            for cid in sorted(set(lab.tolist())):
                ...
                if meanR < best_meanR - 1e-12:
                    best_meanR = meanR; best_cid = int(cid); best_tie_mae = mae_med
                elif abs(meanR - best_meanR) <= 1e-12 and mae_med > best_tie_mae:
                    best_cid = int(cid); best_tie_mae = mae_med
            target_rows.append({
                "algo": algo, "K": k, "target_cluster_id": int(best_cid),
        ...
        for cid in sorted(set(lab.tolist())):
            ...
            stab_rows.append({
                ...
                **{f"size_frac_fold{f}": per_fold_size_frac[f - 1] for f in range(1, 8)},
                ...
                "stability_flag_meanR_cv_ge_0.5": bool(meanR_cv >= 0.5) if not np.isnan(meanR_cv) else None,
    ...
    for drop_feat in base_feats:
        ...
        for k in C.K_VALUES:
            ...
            pert_rows.append({
                ...
                "ari_kmeans_vs_baseline": CL.ari(cluster_assignments[f"K{k}_kmeans"].values, km_drop),
                "ari_hierarchical_vs_baseline": CL.ari(cluster_assignments[f"K{k}_hierarchical"].values, hi_drop),
            })
    ...
    for (algo, k), cid in targets_by_algo_k.items():
        col = ("hdbscan_cluster_id" if algo == "hdbscan"
               else f"K{k}_kmeans" if algo == "kmeans" else f"K{k}_hierarchical")
    ...
    for algo in ("kmeans", "hierarchical"):
        for k in (2,):
            ...
            calib_lines.append(f"PASS at this target: {pass_flag} ({len(cleared)}/8 family members cleared Tier 1 or Tier 2)")
    ...
    calib_lines.append(f"OVERALL CALIBRATION STATUS (PASS = all K=2 targets PASS): "
    ...
    summary_es = es_df.groupby(["algo", "K"])["pass_any_effect_size_metric"].agg(["sum", "count"]).reset_index()
...
def _emit_effect_size_row(out_list, algo, k, cid, mask, sub,
    ...
    out_list.append({
        ...
        "delta_median_fwd_mfe_h24_in_pool_stds": d_mfe24 / pool_mfe24_std if pool_mfe24_std > 0 else float("nan"),
