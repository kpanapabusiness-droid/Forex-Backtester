"""Arc 10 — Step 1 plumbing backtester.

L_ARC_PROTOCOL v2.3 (v2.1.2 base + v2.2 + v2.3 amendments) §5 (Step 1) +
§15a (trades_paths.csv schema). Single pass over the full data window. Produces:

  results/l_arc_10/step1_verbatim/
    trades_all.csv          per-trade summary (Arc 10 schema)
    trades_paths.csv        §15a-compliant per-bar paths, bar_offset 0..240,
                             is_held=1 for entry..actual_exit, is_held=0 for
                             actual_exit+1..entry+240
    prefilter_events.csv    bars passing conditions 1-8 (pre-refractory)
    manifest.json           pool sizes, sha256s, env, configs

Signal (Arc 10 DLR D1 swing-low rejection long) is computed in
``signals/lchar_dlr_long.py`` per the locked v0.1 spec.

Trade mechanics (mirrors scripts/arc_7/step1_backtest.py for §15a parity):
  - Entry: bar N+1 open (long fill = open_mid + spread/2 per SPREAD_SEMANTICS_LOCK).
  - SL: entry_price - 2.0 x Wilder ATR(14)_4H at signal bar N (Step 1 simulation
        default per L_ARC_PROTOCOL §11; per-archetype SL selected at Step 3 SL sweep).
  ...
  - Spread: per-bar from MT5 `spread` column / points_per_pip, floored
        via configs/spread_floors_5ers.yaml.

D1 lag (Arc 10 mandatory): D1 anchor data is loaded per pair from
data/daily/<pair>.csv. The DLR signal module reads only D1 bars whose
date is strictly before the 4H bar t's date, with a further right-edge
constraint that swing-low identifications use bars d <= d_t - 4 (because
swing-low confirmation requires k+3 future D1 bars). NaN-perturbation
test (audits.py) verifies this is structural, not approximated.

Determinism: all floats formatted with "%.10g"; global sort by
(signal_bar_time, pair); trade_ids reassigned in that final order;
manifest written via json.dumps(sort_keys=True). lineterminator='\\n'.

Usage:
    py scripts/l_arc_10/step1_plumbing.py -c configs/wfo_l_arc_10.yaml
    py scripts/l_arc_10/step1_plumbing.py -c configs/wfo_l_arc_10.yaml --verify-determinism
"""
...
# ---------------------------------------------------------------------------
# Wilder ATR(14) at 4H — for SL distance at simulation time.
# ---------------------------------------------------------------------------
...
def _slice_d1_for_4h_window(df_d1: pd.DataFrame, start: str, end: str) -> pd.DataFrame:
    """Slice D1 with a safety window: extra k+1 days at the left (so 4H bars at
    the start can resolve d_t-4 swing-lows) and 1 day at the right (no impact
    on lag, only on the absolute end date)."""
    pad_left = pd.Timedelta(days=10)
    pad_right = pd.Timedelta(days=2)
    start_ts = pd.Timestamp(start) - pad_left
    end_ts = pd.Timestamp(end) + pad_right
    return df_d1[(df_d1["date"] >= start_ts) & (df_d1["date"] <= end_ts)].reset_index(drop=True)
...
def _simulate_pair(
    ...
    L1_arr: np.ndarray,
    L0_arr: np.ndarray,
    L1_age_arr: np.ndarray,
    L0_age_arr: np.ndarray,
    prox_arr: np.ndarray,
    rej_arr: np.ndarray,
    upper_arr: np.ndarray,
    d_t_arr: np.ndarray,
    d_search_max_arr: np.ndarray,
    ...
    for sig_idx in signal_positions:
        ...
        trade = _TradeRow(
            ...
            L1_value=float(L1_arr[sig_idx_int]),
            L0_value=float(L0_arr[sig_idx_int]),
            L1_age_d1_bars=float(L1_age_arr[sig_idx_int]),
            L0_age_d1_bars=float(L0_age_arr[sig_idx_int]),
            L1_to_atr_proximity=float(prox_arr[sig_idx_int]),
            reject_buffer_atr=float(rej_arr[sig_idx_int]),
            upper_fraction=float(upper_arr[sig_idx_int]),
            atr14_at_signal=atr_at_sig,
            d_t_idx=int(d_t_arr[sig_idx_int]),
            d_for_l1_search_max=int(d_search_max_arr[sig_idx_int]),
    ...
    rows = [
        {
            ...
            "L1_value": t.L1_value,
            "L0_value": t.L0_value,
            "L1_age_d1_bars": t.L1_age_d1_bars,
            "L0_age_d1_bars": t.L0_age_d1_bars,
            "L1_to_atr_proximity": t.L1_to_atr_proximity,
            "reject_buffer_atr": t.reject_buffer_atr,
            "upper_fraction": t.upper_fraction,
            "atr14_at_signal": t.atr14_at_signal,
            "d_t_idx": t.d_t_idx,
            "d_for_l1_search_max": t.d_for_l1_search_max,
...
def _write_trades_paths(paths_rows: List[_PathRow], path: Path, float_fmt: str) -> None:
    rows = [
        {
            "trade_id": p.trade_id,
            "pair": p.pair,
    ...
    out = [
        {
            ...
            "L1_value": r.L1_value,
            "L0_value": r.L0_value,
            "L1_age_d1_bars": r.L1_age_d1_bars,
            "L0_age_d1_bars": r.L0_age_d1_bars,
            "L1_to_atr_proximity": r.L1_to_atr_proximity,
            "reject_buffer_atr": r.reject_buffer_atr,
            "upper_fraction": r.upper_fraction,
...
def run(cfg: dict, config_path: Path, *, write_manifest: bool = True) -> Dict[str, Any]:
    ...
    # Locked-param assertions (signal module constants are the source of truth).
    assert int(cfg["signal"]["d1_swing_window_k"]) == sig_mod.D1_SWING_WINDOW_K
    assert int(cfg["signal"]["d1_right_edge_offset"]) == sig_mod.D1_RIGHT_EDGE_OFFSET
    assert int(cfg["signal"]["d1_structure_lookback_bars"]) == sig_mod.D1_STRUCTURE_LOOKBACK_BARS
    assert int(cfg["signal"]["d1_l1_freshness_max_bars"]) == sig_mod.D1_L1_FRESHNESS_MAX_BARS
    assert int(cfg["signal"]["atr_period_4h"]) == sig_mod.ATR_PERIOD_4H
    assert math.isclose(float(cfg["signal"]["proximity_atr_mult"]), sig_mod.PROXIMITY_ATR_MULT)
    assert math.isclose(float(cfg["signal"]["reject_buffer_atr_mult"]), sig_mod.REJECT_BUFFER_ATR_MULT)
    assert math.isclose(float(cfg["signal"]["upper_fraction_min"]), sig_mod.UPPER_FRACTION_MIN)
    assert int(cfg["signal"]["refractory_bars_4h"]) == sig_mod.REFRACTORY_BARS_4H
    ...
    dir_4h = _resolve_data_path(data_dirs["4H"])
    dir_d1 = _resolve_data_path(data_dirs["D1"])

    for pair in pairs:
        df_4h = _slice_window(_load_pair_csv(pair, dir_4h), date_start, date_end)
        df_d1_full = _load_pair_csv(pair, dir_d1)
        df_d1 = _slice_d1_for_4h_window(df_d1_full, date_start, date_end)

        df_sig = sig_mod.compute_signal(df_4h, df_d1, signal_col="signal").reset_index(drop=True)
        signal_mask = df_sig["signal"].to_numpy(dtype=bool)
        prefilter_mask = df_sig["prefilter_pass"].to_numpy(dtype=bool)
        L1_arr = df_sig["L1_value"].to_numpy(dtype=float)
        L0_arr = df_sig["L0_value"].to_numpy(dtype=float)
        L1_age_arr = df_sig["L1_age_d1_bars"].to_numpy(dtype=float)
        L0_age_arr = df_sig["L0_age_d1_bars"].to_numpy(dtype=float)
        prox_arr = df_sig["L1_to_atr_proximity"].to_numpy(dtype=float)
        rej_arr = df_sig["reject_buffer_atr"].to_numpy(dtype=float)
        upper_arr = df_sig["upper_fraction"].to_numpy(dtype=float)
        d_t_arr = df_sig["d_t_idx"].to_numpy(dtype=int)
        d_search_max_arr = df_sig["d_for_l1_search_max"].to_numpy(dtype=int)
        ...
        for idx in pre_positions:
            ...
            all_prefilter.append(
                _PrefilterRow(
                    ...
                    L1_value=float(L1_arr[i]),
                    L0_value=float(L0_arr[i]),
                    L1_age_d1_bars=float(L1_age_arr[i]),
                    L0_age_d1_bars=float(L0_age_arr[i]),
                    L1_to_atr_proximity=float(prox_arr[i]),
                    reject_buffer_atr=float(rej_arr[i]),
                    upper_fraction=float(upper_arr[i]),
        ...
        trades, paths, fired, emitted, skipped = _simulate_pair(
            ...
            L1_arr=L1_arr,
            L0_arr=L0_arr,
            L1_age_arr=L1_age_arr,
            L0_age_arr=L0_age_arr,
            prox_arr=prox_arr,
            rej_arr=rej_arr,
            upper_arr=upper_arr,
            d_t_arr=d_t_arr,
            d_search_max_arr=d_search_max_arr,
    ...
    sha_arc10_cfg = _sha256_file(config_path)
    sha_signal_module = _sha256_file(_REPO_ROOT / "signals" / "lchar_dlr_long.py")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Arc 10 Step 1 plumbing backtester.")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.verify_determinism:
        ...
        t = info_run2["totals"]
        print(
            f"[arc_10 step 1] fired={t['total_signals_fired']} "
        ...
        print(f"[arc_10 step 1] determinism: {'PASS' if determinism_pass else 'FAIL'}")
