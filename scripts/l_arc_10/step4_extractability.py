"""Arc 10 — Step 4 extractability investigation.
...
``scripts.l_arc_10.step3_capturability._eval_trade_at_sl`` for byte-identity
with Step 3.
...
Usage:
    py scripts/l_arc_10/step4_extractability.py -c configs/l_arc_10/step4.yaml
"""
...
PIPELINE_E_FEATURES: List[str] = [
    ...
    # Pair (integer-coded)
    "pair_id_int",
    # Arc-10-specific DLR (HTF context + 4H reject geometry)
    "L1_to_atr_proximity",          # (low[t] - L1) / ATR
    "reject_buffer_atr",            # (close[t] - L1) / ATR
    "upper_fraction",               # (close - low) / (high - low) at signal bar
    "L1_age_d1_bars",               # how recent is L_1 (1..20 D1 bars)
    "L0_age_d1_bars",               # how recent is L_0 (older than L_1)
    "L1_minus_L0_atr",              # (L1 - L0) / ATR — D1 HL slope magnitude
    "L1_minus_L0_d1_bars",          # L_0 age - L_1 age = D1 bar distance L_0->L_1
...
def compute_pipeline_e_features(
    ...
    for _, t in trades_df.iterrows():
        ...
        # Arc-10 DLR signal features (HTF context + 4H reject geometry)
        l1_val = float(t.get("L1_value", float("nan")))
        l0_val = float(t.get("L0_value", float("nan")))
        l1_age = float(t.get("L1_age_d1_bars", float("nan")))
        l0_age = float(t.get("L0_age_d1_bars", float("nan")))
        l1_to_atr_proximity = float(t.get("L1_to_atr_proximity", float("nan")))
        reject_buffer_atr = float(t.get("reject_buffer_atr", float("nan")))
        upper_fraction = float(t.get("upper_fraction", float("nan")))
        l1_minus_l0_atr = (
            (l1_val - l0_val) / atr_t
            if (atr_t > 0 and not math.isnan(l1_val) and not math.isnan(l0_val))
            else float("nan")
        )
        l1_minus_l0_d1_bars = l0_age - l1_age if not (math.isnan(l1_age) or math.isnan(l0_age)) else float("nan")
        ...
        rows.append({
            ...
            "L1_age_d1_bars": l1_age,
            "L0_age_d1_bars": l0_age,
            "L1_minus_L0_atr": l1_minus_l0_atr,
            "L1_minus_L0_d1_bars": l1_minus_l0_d1_bars,
...
def _full_fit_and_threshold(
    ...
    # v2.2 §3 (Arc 10): select max precision with recall >= recall_min.
    # NO max-F1 fallback. If no threshold satisfies recall_min, archetype dies.
    recall_min = float(sweep_cfg.get("recall_min", 0.60))
    ...
    while t <= t_max + 1e-9:
        admit_mask = p >= t
        tp = int(((admit_mask) & (y == 1)).sum())
        fp = int(((admit_mask) & (y == 0)).sum())
        fn = int(((~admit_mask) & (y == 1)).sum())
        ...
        sweep_row = (float(t), float(admission), float(exclusion),
                     float(real_succ), float(lift), float(precision), float(recall))
        sweep.append(sweep_row)
        if recall >= recall_min:
            candidates.append(sweep_row)
        t += t_step
    ...
    if candidates:
        # v2.2 §3: max precision subject to recall >= recall_min.
        # Ties broken by larger t (more stringent).
        candidates.sort(key=lambda r: (-r[5], -r[0]))
        best = candidates[0]
        selected_t = best[0]
        adm_at_t = best[1]
        excl_at_t = best[2]
        real_at_t = best[3]
        lift_at_t = best[4]
    # If no candidate satisfies recall >= recall_min, selected_t stays None
    # and the unit's caller treats that as a Step 4 fail (per v2.2 §3).
...
def write_threshold_sweep(out_path: Path, r: PipelineResult) -> None:
    with out_path.open("w", encoding="utf-8", newline="") as f:
        w = csv.writer(f, lineterminator="\n")
        w.writerow(["t", "admission", "exclusion", "realised_success", "lift", "precision", "recall"])
        for row in r.threshold_sweep:
            pad = row + (float("nan"),) * (7 - len(row))
            w.writerow([_fmt(pad[0]), _fmt(pad[1]), _fmt(pad[2]), _fmt(pad[3]), _fmt(pad[4]), _fmt(pad[5]), _fmt(pad[6])])
...
def write_summary_md(
    ...
    lines: List[str] = []
    lines.append("# Arc 10 — Step 4 extractability summary")
    lines.append("")
    ...
    lines.append("- `configs/l_arc_10/step4.yaml`")
    lines.append("- `scripts/l_arc_10/step4_extractability.py`")
    lines.append("")
...
def _run_once(cfg: dict) -> Tuple[Dict[str, str], Dict[str, Any]]:
    ...
    pair_caches: Dict[str, PerPairCache] = {}
    for p in pairs:
        print(f"[l_arc_10 step4] caching {p}", file=sys.stderr)
        pair_caches[p] = _build_pair_cache(p, cfg["data"]["dir_4h"], cfg["data"]["dir_d1"])

    # Features.
    print("[l_arc_10 step4] computing pipeline E features", file=sys.stderr)
    e_features = compute_pipeline_e_features(trades_df, pair_caches)
    print("[l_arc_10 step4] computing pipeline D1 features (with lag audit)", file=sys.stderr)
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Arc 10 Step 4 — extractability.")
    ap.add_argument("-c", "--config", type=Path, default=Path("configs/l_arc_10/step4.yaml"))
    ...
    print("[l_arc_10 step4] === RUN 1 ===", file=sys.stderr)
    hashes1, ctx = _run_once(cfg)
    hashes2 = None
    if bool(cfg["output"].get("determinism_check", True)):
        print("[l_arc_10 step4] === RUN 2 (determinism) ===", file=sys.stderr)
    ...
    print(f"[l_arc_10 step4] DONE. survivors={n_pass}, det={det_gate}", file=sys.stderr)
