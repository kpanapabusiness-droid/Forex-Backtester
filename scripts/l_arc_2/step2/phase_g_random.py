...
from scripts.l_arc_2.step2._io import (
    PAIRS, SPREAD_FLOOR_PATH, STEP2_DIR, VERBATIM_SL_ATR_MULT, VERBATIM_TIME_EXIT_H,
    hash_seed, load_all_floors, load_pair_1h, load_trades_verbatim, pip_size, wilder_atr,
)
...
def run_phase_g() -> None:
    ...
    seed_root = hash_seed("l_arc_2_step2_random_baseline")
    rand_rows: List[dict] = []
    for pair in PAIRS:
        ...
        for fold_id, oos_start, oos_end in FOLD_BOUNDARIES:
            ...
            if k == 0:
                continue
            seed_pf = hash_seed(f"l_arc_2_step2_random__{pair}__fold{fold_id}__h{VERBATIM_TIME_EXIT_H}")
