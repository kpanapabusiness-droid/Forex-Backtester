...
from scripts.l_arc_2.step2._io import (
    FORWARD_HORIZON_BARS_DEFAULT, FORWARD_HORIZON_BARS_EXTENDED, H_GRID,
    STEP1_DIR, STEP2_DIR, sha256_file,
)
...
def write_sanity_checks(extended: bool = False) -> Path:
    ...
    checks.append({"id": 4, "check": "4 new arc-2 pre-signal context features present",
    ...
    checks.append({"id": 14, "check": "Determinism: two-consecutive-run byte-identical",
                   "expected": "PASS",
                   "observed": "PASS" if det_pass else "PENDING (run scripts/l_arc_2/step2/run_determinism_check.py)",
    ...
    lines = ["L Arc 2 Step 2 — Sanity Checks (op spec §4 step 2 + task validation)",
             "=" * 70, "",
...
def write_run_manifest() -> Path:
    in_files = {
        "config (configs/wfo_l_arc2_verbatim.yaml)": REPO_ROOT / "configs" / "wfo_l_arc2_verbatim.yaml",
    ...
    lines = ["L Arc 2 Step 2 — Run Manifest", "=" * 60, "",
             "## Inputs (sha256)"]
...
def write_phase_doc(extended: bool) -> Path:
    ...
    md.append("| Input trade-set | `results/l_arc_2/step1_verbatim/trades_verbatim.csv` (3,993 rows) |")
    ...
    md.append("  - 1H signal TF, mtf_alignment family, 2_down_mixed base, kijun sub-spec, h=120 horizon.")
    md.append("")
    ...
    md.append(det_text if det_text != "<missing>" else
              "(see determinism_check.txt — produced by scripts/l_arc_2/step2/run_determinism_check.py)")
    md.append("```")
    md.append("")
    ...
    md.append("")
    md.append(f"- `results/l_arc_2/step2_descriptive/signals_features.csv` "
    ...
    md.append(f"- `results/l_arc_2/step2_descriptive/trade_paths.csv` ({paths_count} rows; "
    ...
    md.append(f"~{100*(f['exit_reason']=='data_end').mean():.2f}% data_end (8 fold-7 tail trades).")
    ...
    md.append("- 8 fold-7 data_end trades carried through with `data_end_flag=True`. Excluded from "
