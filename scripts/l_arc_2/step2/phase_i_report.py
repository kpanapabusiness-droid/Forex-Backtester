...
"""Phase I — write PHASE_L_ARC_2_STEP2.md, step2_sanity_checks.txt, run_manifest.txt.
...
def write_sanity_checks(extended: bool = False) -> Path:
    ...
    checks.append(
        {
            "id": 1,
            "check": "signals_features.csv rows = trades_verbatim.csv rows",
            "expected": n_trades_raw,
            "observed": n_feat,
    ...
    paths_expected = (
        int(f["forward_window_bars_available"].sum())
        if "forward_window_bars_available" in f.columns
        else (n_feat * H)
    )
    checks.append(
        {
            "id": 2,
            "check": "trade_paths.csv row count = sum of available forward bars",
            "expected": paths_expected,
            "observed": n_paths_lines,
    ...
    checks.append(
        {
            "id": 3,
            "check": "All op spec §5 subfolders present",
    ...
    checks.append(
        {
            "id": 4,
            "check": "4 new arc-2 pre-signal context features present",
            "expected": "all 4 present",
            "observed": "all 4 present"
            if psc_present
            else f"missing: {[c for c in new_psc if c not in f.columns]}",
    ...
    checks.append(
        {
            "id": 5,
            "check": "3 Amendment 4 clustering features present",
            "expected": "all 3 present",
            "observed": "all 3 present"
            if amd4_present
            else f"missing: {[c for c in amd4 if c not in f.columns]}",
    ...
    checks.append(
        {
            "id": 6,
            "check": "Lookahead-invariant features test",
            "expected": "PASS",
            "observed": "PASS" if look_pass else "FAIL",
    ...
    checks.append(
        {
            "id": 7,
            "check": "Feature lag audit (op spec §10.4)",
            "expected": "PASS",
            "observed": "PASS" if feat_lag_pass else "FAIL",
    ...
    checks.append(
        {
            "id": 8,
            "check": "Forward-horizon stability check produced",
            "expected": "documented",
            "observed": "documented" if stab_present else "missing",
    ...
    checks.append(
        {
            "id": 9,
            "check": "Random-entry baseline produced; differs documented",
            "expected": "produced",
    ...
    checks.append(
        {
            "id": 10,
            "check": "Sample-size discipline applied (n<30 flag column)",
            "expected": "applied",
            "observed": "applied" if sample_ok else "missing",
    ...
    checks.append(
        {
            "id": 11,
            "check": "Distribution shape per §11.1",
            "expected": "full shape",
    ...
    checks.append(
        {
            "id": 13,
            "check": "Per-fold breakdowns (§11.3)",
            "expected": ">=20",
            "observed": f"{by_fold}",
    ...
    checks.append(
        {
            "id": 14,
            "check": "Determinism: two-consecutive-run byte-identical",
            "expected": "PASS",
            "observed": "PASS"
            if det_pass
            else "PENDING (run scripts/l_arc_2/step2/run_determinism_check.py)",
            "status": "PASS" if det_pass else "PENDING",
        }
    )

    phase_doc = STEP2_DIR / "PHASE_L_ARC_2_STEP2.md"
    checks.append(
        {
            "id": 15,
            "check": "Phase doc descriptive only (op spec §11.5)",
    ...
    lines = [
        "L Arc 2 Step 2 — Sanity Checks (op spec §4 step 2 + task validation)",
        "=" * 70,
        "",
...
def write_run_manifest() -> Path:
    in_files = {
        "config (configs/wfo_l_arc2_verbatim.yaml)": REPO_ROOT
        / "configs"
        / "wfo_l_arc2_verbatim.yaml",
        "spread_floor (configs/spread_floors_5ers.yaml)": REPO_ROOT
        / "configs"
        / "spread_floors_5ers.yaml",
    ...
    lines = ["L Arc 2 Step 2 — Run Manifest", "=" * 60, "", "## Inputs (sha256)"]
    ...
    for p in out_files:
        if p.exists():
            ...
            lines.append(
                f"  <missing>                                                          {p.relative_to(STEP2_DIR).as_posix()}"
            )
    lines += [
        "",
        "## Determinism",
...
def write_phase_doc(extended: bool) -> Path:
    ...
    # Pre-signal context (arc-2 new)
    psc_cols = ["cum_logret_1h_24", "cum_logret_1h_72", "cum_logret_1h_168", "vol_realized_1h_24h"]
    psc_summary = {
        c: {
            "p50": _safe_median(f[c]),
            "mean": _safe_mean(f[c]),
            "p5": _safe_quantile(f[c], 0.05),
            "p95": _safe_quantile(f[c], 0.95),
        }
        for c in psc_cols
    }
    ...
    crossing_text = _read_text_or_none(
        STEP2_DIR / "cost_stress" / "_crossing_zero_note.txt"
    ).strip()
    rand_diff_text = _read_text_or_none(
        STEP2_DIR / "random_baseline" / "differs_from_verbatim.txt"
    ).strip()
    ...
    md: List[str] = []
    md.append("# PHASE_L_ARC_2_STEP2 — Descriptive Trade-Path Analysis")
    md.append("")
    md.append("## 1. Header")
    md.append("")
    md.append("| Field | Value |")
    md.append("|---|---|")
    md.append("| Arc | L Arc 2 — standalone signal arc (no calibration-check role) |")
    md.append("| Step | 2 — Descriptive Trade-Path Analysis (NOT a gate) |")
    md.append(
        "| Protocol | `L_ARC_PROTOCOL.md` v1.0 + `L_ARC_PROTOCOL_v1.1_AMENDMENT.md` (12 amendments) |"
    )
    md.append("| Operational spec | `L_ARC_OPERATIONAL_SPEC.md` v1.0 |")
    md.append("| Arc-open doc | `results/l_arc_2/PHASE_L_ARC_2_OPEN.md` |")
    md.append("| Predecessor doc | `results/l_arc_2/step1_verbatim/PHASE_L_ARC_2_STEP1.md` |")
    md.append(
        "| Input trade-set | `results/l_arc_2/step1_verbatim/trades_verbatim.csv` (3,993 rows) |"
    )
    md.append(
        f"| Forward horizon | H = {H}"
        + (
            " (extended from default 240 per Phase D)"
            if extended
            else " (default; stability check did not trigger extension)"
        )
        + " |"
    )
    ...
    md.append("")
    md.append("Signal: `TRIAL__mtf_alignment__2_down_mixed__kijun__h_120`")
    md.append(
        "  - 1H signal TF, mtf_alignment family, 2_down_mixed base, kijun sub-spec, h=120 horizon."
    )
    md.append("")
    ...
    md.append("")
    md.append(
        "- **Identity / context:** `trade_id, pair, fold_id, signal_bar_ts, "
        "entry_bar_ts, exit_bar_ts, direction`"
    )
    md.append(
        "- **Signal-bar 1H properties:** open/close/high/low, log_return, "
        "abs_log_return, atr_at_signal_1h, atr_baseline_1h_200, atr_ratio_to_baseline, "
        "signal_bar_volume, signal_bar_volume_nan"
    )
    md.append(
        "- **Pre-signal 1H context (legacy):** cum_logret_1h_3, cum_logret_1h_6, "
        "dist_close_to_high30_atr, dist_close_to_low30_atr"
    )
    md.append("- **Pre-signal 1H context (arc-2 NEW, v1.1 amendment):** ")
    ...
    md.append(
        "- **Time / session / liquidity:** hour_utc, day_of_week, session, "
        "hour_in_4h_bar, bars_to_next_4h_close, hour_in_d1_bar, bars_to_next_d1_close"
    )
    md.append(
        "- **First-bar (N+1) properties:** first_bar_direction, first_bar_range_atr, first_bar_range_bin"
    )
    md.append(
        "- **Regime / momentum bins:** vol_regime, pre_momentum_bin, "
        "cum_logret_1h_{6,24,168}_bin, vol_realized_1h_24h_decile, trigger_magnitude_decile"
    )
    md.append(
        "- **Cross-pair / portfolio (§5.15):** concurrent_signals_same_bar, "
    ...
    md.append(
        "- **Trade outcome:** net_r, gross_r, spread_cost_R, mfe_R, mae_R, bars_held, "
    ...
    md.append("- **HELD-window path aggregates (REAL on arc 2 — held window > 1 bar):**")
    ...
    md.append(
        f"- **bars_to thresholds (capped at H={H}):** "
    ...
    md.append(
        "  `fwd_realized_range_atr, fwd_fraction_time_above_entry, fwd_max_consecutive_directional_bars`"
    )
    md.append("")
    ...
    md.append("")
    md.append(
        "- **Columns:** `trade_id, bar_offset, bar_ts, open, high, low, close, "
    ...
    md.append(
        f"- **bar_offset range:** [0, {H - 1}] (forward window). is_forward_bar=True throughout."
    )
    ...
    md.append("")
    md.append(
        f"Pool `net_r`: p50={p50_net_r:.4f}, mean={mean_net_r:.5f}, "
    ...
    md.append("")
    md.append(
        f"Forward path geometry at h=24: p50 fwd_mfe = {p50_fwd_mfe_h24:.3f} ATR, "
    ...
    md.append("")
    md.append(
        "Held-window class counts and per-class summaries in "
    ...
    md.append("")
    md.append(
        f"HELD-window: oscillation_count p50 {p50_osc:.0f}, monotonicity_ratio p50 {p50_mono:.3f}, "
    ...
    md.append("")
    md.append(
        "Survival curve in `distributions/survival/survival.csv`. Reports the fraction of "
    ...
    md.append("")
    md.append(
        "Correlation between cum_R at bar_offset (t-1) and final net R reported in "
    ...
    md.append("")
    md.append(
        "Per-side summary in `distributions/asymmetry/win_loss_asymmetry.csv` with median R, "
    ...
    md.append("")
    md.append(
        "Per-stratum summaries (n, mean, std, p5..p95, max) for 17 metrics across 17 "
        "stratification axes — including the arc-2 new pre-signal context axes "
    ...
    n_joint = sum(1 for _ in joint_dir.glob("*.csv")) if joint_dir.exists() else 0
    md.append(
        f"{n_joint} 2D heatmaps (binned-count crosstabs, op spec §11.2) in `joint_distributions/`. "
        ...
        "vol_regime×fwd_mfe_h24, and four arc-2 new-feature heatmaps "
    ...
    md.append("")
    md.append(
        "**Time-exit sweep** (bar_offset=1, SL=2.0 ATR, h ∈ {1, 3, 6, 12, 24, 48, 120, 240}):"
    )
    md.append("")
    md.append("```")
    md.append(
        te_curve[
            [
                ...
                "frac_sl_hit",
                "frac_time_exit",
                "median_capture_ratio_vs_fwd_mfe",
            ]
        ].to_string(index=False)
    )
    md.append("```")
    md.append("")
    md.append(
        "Full per-shadow per-trade CSVs under `shadow_tradesets/<axis>/`; pool + per-fold "
    ...
    md.append("")
    md.append("```")
    md.append(rand_diff_text)
    md.append("```")
    md.append("")
    md.append(
        "Comparison detail in `random_baseline/comparison.csv`; per-trade random-baseline "
        "outcomes in `random_baseline/random_entry_distribution.csv`. Seeds are hash-based per "
        "Amendment 11 (`hash_seed('l_arc_2_step2_random_baseline')` xor per-pair-per-fold key)."
    )
    md.append("")
    md.append("### 6.14 Held-bar context evolution (§5.14)")
    md.append("")
    md.append(
        "Per-trade per-t (t ∈ {1, 3, 5, 10, 20}) samples of currency-basket cum log return, "
        ...
        "at bar_offset t contribute; arc 2's mean bars_held ~47 means held rows thin out at "
    ...
    md.append("")
    md.append(
        f"`concurrent_signals_within_3h`: mean {_safe_mean(csw):.2f}, p50 {_safe_median(csw):.0f}, "
        f"p90 {_safe_quantile(csw, 0.90):.0f}, p95 {_safe_quantile(csw, 0.95):.0f}, "
        f"p99 {_safe_quantile(csw, 0.99):.0f}, max {float(csw.max()):.0f}. "
    )
    md.append(
        "Other §5.15 features (concurrent_signals_same_bar, currency_basket_3h_{USD,EUR,JPY,GBP}, "
    ...
    md.append("")
    md.append("## 8. Pre-signal context (arc-2 NEW) summary")
    md.append("")
    ...
    md.append(
        det_text
        if det_text != "<missing>"
        else "(see determinism_check.txt — produced by scripts/l_arc_2/step2/run_determinism_check.py)"
    )
    md.append("```")
    md.append("")
    ...
    md.append("")
    md.append("- **8 fold-7 data_end trades** preserved per step 1 handover. Their forward-window")
    md.append(f"  is truncated below H={H} at the end of available data; affected aggregates")
    md.append("  appear as NaN. They contribute to pool path-complexity and held-window")
    md.append("  distributions through their available bars. Flagged via `data_end_flag=True` in")
    md.append("  signals_features.csv and via `data_end_flag` in the relevant trade_paths rows.")
    ...
    md.append(
        "- **Cross-pair dispersion proxy.** Implemented as per-bar std of 28 pair-log-returns,"
    )
    ...
    md.append("")
    md.append(
        f"- `results/l_arc_2/step2_descriptive/signals_features.csv` "
    ...
    md.append(
        f"- `results/l_arc_2/step2_descriptive/trade_paths.csv` ({paths_count} rows; "
    ...
    md.append(
        f"expanding folds (Oct 2020 – Jan 2026), held a mean of {mean_bars_held:.1f} bars each"
    )
    md.append(
        f"(median {p50_bars_held:.0f}; max 120) under verbatim execution. Pool median net R is"
    )
    md.append(
        f"{p50_net_r:.4f}; mean {mean_net_r:.5f}. Exit mix is "
        f"~{100 * (f['exit_reason'] == 'sl_hit').mean():.0f}% sl_hit, "
        f"~{100 * (f['exit_reason'] == 'time_exit').mean():.0f}% time_exit, "
    )
    md.append(
        f"~{100 * (f['exit_reason'] == 'data_end').mean():.2f}% data_end (8 fold-7 tail trades)."
    )
    md.append(
        f"Forward path geometry at h=24: p50 fwd_mfe = {p50_fwd_mfe_h24:.2f} ATR, p50 fwd_mae"
    )
    md.append(
        f"= {p50_fwd_mae_h24:.2f} ATR. Race-condition p50 = {race_p[50]:.1f} bars (positive = "
    )
    ...
    te_shape = (
        "monotonic"
        if (te_mean_r.diff().dropna() > 0).all() or (te_mean_r.diff().dropna() < 0).all()
        else "non-monotonic"
    )
    md.append("Time-exit curve shape (mean R as a function of h, descriptive only):")
    md.append(
        f"  peak at h={te_peak_h} (mean R {te_mean_r[te_peak_h]:.4f}); "
    ...
    md.append(
        f"  peak at SL={sl_peak_m} ATR (mean R {sl_mean_r[sl_peak_m]:.4f}); "
        f"trough at SL={sl_min_m} ATR (mean R {sl_mean_r[sl_min_m]:.4f})."
    )
    md.append("")
    md.append(
        f"Random baseline summary: {rand_diff_text.splitlines()[0] if rand_diff_text else 'see random_baseline/differs_from_verbatim.txt'}."
    )
    md.append(
        f"Forward-horizon stability outcome: H = {H}"
        + (" (extended)" if extended else " (default; not extended)")
        + "."
    )
    md.append("")
    md.append("**Cross-arc carries from arc 1 (per arc-open §4) — discharged at step 2:**")
    md.append("")
    md.append(
        "- 4 pre-signal context columns added to `signals_features.csv` "
        "(`cum_logret_1h_24/72/168, vol_realized_1h_24h`). Lookahead-safe; see §9."
    )
    md.append(
        "- §5.15 cross-pair / portfolio family present (concurrent_signals, currency_basket, "
        "trade_overlap, sequential_same_pair_density)."
    )
    md.append("- Time-exit shape descriptive summary above.")
    md.append(
        "- 3 Amendment 4 clustering features added (fwd_realized_range_atr, "
    ...
    md.append("")
    md.append(
        "- The clustering feature subset (op spec §6.1) for step 3 phase A should read: "
    ...
    md.append(
        "- Per-stratum cells with n<30 are flagged in conditional_breakdowns/<stratum>/<metric>.csv "
        "via `flagged_n_lt_30` column; n<10 pooled into `_insufficient_n` rows."
    )
    md.append(
        "- 8 fold-7 data_end trades carried through with `data_end_flag=True`. Excluded from "
        "exit-reason percentages that pool by clean exit class; included in pool path "
        "distributions; their forward window is truncated."
    )
    md.append("")

    out = STEP2_DIR / "PHASE_L_ARC_2_STEP2.md"
...
def main(extended: bool = False) -> None:
    ...
    print("[Phase I] writing PHASE_L_ARC_2_STEP2.md...")
