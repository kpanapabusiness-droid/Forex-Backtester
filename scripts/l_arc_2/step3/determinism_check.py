"""Two-run sha256-identical determinism check for L Arc 2 Step 3.
...
OUT = REPO / "results" / "l_arc_2" / "step3_extractability"
...
def main() -> int:
    ...
    rc = subprocess.run(
        [sys.executable, "-m", "scripts.l_arc_2.step3.run_step3"],
        cwd=str(REPO),
    ).returncode
    if rc != 0:
    ...
    lines = [
        "L Arc 2 Step 3 — Two-consecutive-run determinism check",
        "=" * 70,
        "",
