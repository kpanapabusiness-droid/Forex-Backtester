...
def run_exit_only_h240(signals: pd.DataFrame, paths: pd.DataFrame) -> pd.DataFrame:
    ...
    sim = S.simulate_time_exit_h(
        horizon=240, trade_ids=tids, signals=signals, paths_long=paths,
    )
...
def run_exit_cluster_cond(
    ...
    if len(c1_tids) > 0:
        sim_c1 = S.simulate_close_at_bar_t(
            t=t, trade_ids=c1_tids, signals=signals_with_clu, paths_long=paths,
        )
...
def run_exit_cluster_cond_h240(
    ...
    if len(c0_tids) > 0:
        sim_c0 = S.simulate_time_exit_h(
            horizon=240, trade_ids=c0_tids, signals=signals_with_clu, paths_long=paths,
        )
...
def run_delayed_entry(
    ...
    if len(c0_tids) == 0:
        return pd.DataFrame(columns=[
            ...
            "exit_reason", "net_r", "gross_r", "spread_cost_r",
            "mfe_at_exit", "mae_at_exit",
        ])

    sim = S.simulate_delayed_entry(
        t=t, trade_ids=c0_tids, signals=signals_with_clu, paths_long=paths,
