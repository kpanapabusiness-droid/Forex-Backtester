"""Shared I/O + path helpers for L Arc 3 Step 2.

Arc 3 signal: volatility_regime / d1_atr_top_decile / any / h=120
(D1-ATR-regime-conditioned; first regime-conditional signal in the L registry).

Arc 3 differs from arc 2 in:
  - Signal mechanism: regime-conditional (D1 vol top decile) vs state-conditional (mtf-alignment 2_down_mixed)
  - Trade count: 2,770 (vs arc 2 3,993) — cap binds harder due to ~37x longer fire-run lengths
  - Exit mix: sl_hit 72.85% / time_exit 27.15% / data_end 0 (vs arc 2 76.03/23.77/0.20)
  - Pool mean net R: +0.036 (vs arc 2 -0.019)
  - Arc-3-specific signal columns in trades_verbatim.csv: d1_atr14_simple_at_signal, d1_atr14_decile_rank
  - Arc-3-specific signals_features columns added in step 2: same two + signal_bar_direction
  - 4 pre-signal context features inherited from arc 2 (cum_logret_1h_{24,72,168}, vol_realized_1h_24h)
  - 3 Amendment 4 clustering features inherited (fwd_realized_range_atr, fwd_fraction_time_above_entry,
...
STEP1_DIR = REPO_ROOT / "results" / "l_arc_3" / "step1_verbatim"
...
# Verbatim time-exit horizon (arc 3 registry entry 2)
...
RANDOM_SEED: int = hash_seed("l_arc_3_step2_root")
...
def floor_pips_for_pair(
    spread_floor_yaml: Path, pair: str, points_per_pip: float = POINTS_PER_PIP
) -> Optional[float]:
...
def load_all_floors(
    spread_floor_yaml: Path = SPREAD_FLOOR_PATH, points_per_pip: float = POINTS_PER_PIP
) -> Dict[str, float]:
    data = yaml.safe_load(spread_floor_yaml.read_text(encoding="utf-8")) or {}
    out: Dict[str, float] = {}
...
def histogram_csv(
    values, *, bins: int = 50, bin_range: Optional[tuple[float, float]] = None
) -> pd.DataFrame:
    arr = np.asarray(list(values), dtype=float)
    finite = arr[np.isfinite(arr)]
    if finite.size == 0:
