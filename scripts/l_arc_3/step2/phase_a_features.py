"""Phase A — feature augmentation for L Arc 3 step 2.

Emits:
  - signals_features.csv  (one row per taken trade; full feature set per op spec §5.16
                           + v1.0 additions + arc-3 pre-signal context (4) + Amendment 4 (3)
...
Conventions:
  - Arc 3 verbatim execution: enter at bar N+1 open, hold up to h=120 bars (exit at bar
...
def compute_path_aggregates(
    ...
    `held_bars` is the verbatim trade's bars_held (1..120 for arc 3). Held aggregates
    ...
    # ---- HELD-window path complexity (real on arc 3; held_bars > 1) ----
    if held_bars >= 2:
...
def _pre_signal_context(close_arr: np.ndarray, sig_idx: int) -> dict:
    """Compute the 4 arc-3 pre-signal context features (cum_logret_1h_24/72/168, vol_realized_1h_24h).
...
def run_phase_a(H: int = FORWARD_HORIZON_BARS_DEFAULT) -> dict:
    ...
    # Arc-3-specific signal columns inherited from trades_verbatim.csv.
    tr_d1_atr14_simple = trades["d1_atr14_simple_at_signal"].astype(float).to_numpy()
    tr_d1_atr14_decile = trades["d1_atr14_decile_rank"].astype(float).to_numpy()
    ...
    for k in range(n_trades):
        ...
        row = {
            ...
            # arc-3 signal-side D1 ATR features (regime-conditioning axis)
            "d1_atr14_simple_at_signal": float(tr_d1_atr14_simple[k]),
            "d1_atr14_decile_rank": float(tr_d1_atr14_decile[k]),
            # arc-3 signal-bar direction (the "any" sub-spec lets up- and down-bars both fire)
            "signal_bar_direction": (
                "up" if sig_close > sig_open else ("down" if sig_close < sig_open else "doji")
            ),
            # pre-signal context (legacy)
            "cum_logret_1h_6": cum_logret_6,
            "cum_logret_1h_3": cum_logret_3,
            # pre-signal context (arc-3 NEW — v1.1 amendment)
    ...
    # Trigger magnitude decile from signal_bar_abs_log_return (arc-3 proxy: signal-bar move magnitude)
