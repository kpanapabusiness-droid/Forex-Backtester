...
from scripts.l_arc_3.step2.phase_a_features import (
    _pre_signal_context,
...
def run_lookahead_test(H: int = FORWARD_HORIZON_BARS_DEFAULT,
    ...
    perturb_seed_base = hash_seed("l_arc_3_step2_perturb")
...
def write_feature_lag_audit(H: int = FORWARD_HORIZON_BARS_DEFAULT) -> Path:
    ...
    Extended to cover the 4 new arc-3 pre-signal context features and the 3
    Amendment 4 forward-window-derived clustering features.
    """
    out = STEP2_DIR / "feature_lag_audit.txt"
    # Deterministic 100-row sample
    trades = load_trades_verbatim()
    rng = np.random.default_rng(hash_seed("l_arc_3_step2_lag_audit"))
    ...
    lines = [
        ...
        "",
        "Pre-signal context (NEW in arc 3, v1.1 amendment) — 4 features:",
        ...
        "  - LOOKAHEAD ASSERT: in scripts/l_arc_3/step2/phase_a_features.py::",
        ...
        "volatility_regime_bar_identity_check.txt establish this independent of step 2.",
