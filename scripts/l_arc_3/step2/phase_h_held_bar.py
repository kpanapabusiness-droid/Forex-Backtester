...
Arc 3 has a real held window (mean ~47 bars, max 120), so for each open trade
we sample contextual features at t ∈ {1, 3, 5, 10, 20}. A trade contributes
to t iff bars_held > t (i.e. still in the held window at bar_offset t).
...
def run_phase_h() -> None:
    ...
    held_rows_per_t: Dict[int, List[dict]] = {t: [] for t in HEALD_T}
    fwd_rows_per_t: Dict[int, List[dict]] = {t: [] for t in HEALD_T}
    ...
    for k in range(len(features)):
        ...
        for t in HEALD_T:
            ...
            # Held only if still open at bar_offset t  (t indexed from 1; offset = t-1)
            if t <= bars_held:
                held_rows_per_t[t].append(row)
            # Forward always (unconditional on exit)
            fwd_rows_per_t[t].append(row)
    ...
    note.write_text(
        ...
        "returns across the 28 pairs at each bar timestamp. Low values indicate pairs\n"
        "moving together (high cross-pair correlation); high values indicate dispersion.\n"
