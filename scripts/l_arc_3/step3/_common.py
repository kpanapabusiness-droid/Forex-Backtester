"""Shared utilities for L Arc 3 Step 3 extractability scripts.

Arc 3 differs from arc 1:
...
- Signal-time predictor set includes arc-3 NEW pre-signal context (cum_logret_1h_24/72/168,
...
STEP2_DIR = REPO / "results" / "l_arc_3" / "step2_descriptive"
...
OUT_DIR = REPO / "results" / "l_arc_3" / "step3_extractability"
...
# Arc 3 has NO signal_threshold_q90 / trigger_excess / trigger_ratio columns;
...
# Categorical signal-time predictors (one-hot encoded). Arc 3 adds the 3 new bin
# categoricals (cum_logret_1h_6_bin already existed for arc 1; arc 3 also has 24 + 168).
...
#
# Arc 3 has REAL held-window features (mean bars_held ~47, not degenerate).
...
CLUSTER_FEATURES_NUMERIC = (
    ...
    "oscillation_count",
    "monotonicity_ratio",
    # Amendment 4 three (already in signals_features.csv on arc 3)
    "fwd_realized_range_atr",
    "fwd_fraction_time_above_entry",
    "fwd_max_consecutive_directional_bars",
)
...
# Reported regardless of tier outcome per arc-3 open doc.
