"""Arc 8 v3.0.2 Step 2 — path-shape clustering.

Per L_PROTOCOL §2 Step 2 + intent doc §2:
...
Operates on Step 1's pool.parquet + paths.parquet, which were produced
under `boundary_convention="5ers_eet"`. The clustering itself is
convention-agnostic — path-shape features are R-unit derived from each
trade's own forward path, independent of bar-storage convention.

Mirrors prior _halted's `scripts/l_arc_8_v3_0_2/step_2_clustering.py`.
"""
...
from scripts.l_arc_8_v3_0_2.shared import ARC_NAME, RESULTS_ROOT
...
def _path_feats_per_trade(paths_df: pd.DataFrame, trade_pool: pd.DataFrame) -> pd.DataFrame:
    ...
    for tid, grp in paths_sorted.groupby("trade_id", sort=True):
        ...
        dd_depth = float(np.min(low_r - running_peak))
...
def _shape_tag(row: pd.Series, q: dict) -> str:
    """Assign archetype label per quartile rules (Arc 8 original)."""
...
def _build_summary_md(
    ...
    lines = [
        f"# {ARC_NAME} — Step 2 Cluster Summary",
        "",
        ...
        "",
        "Arc 8 v3.0 surfaced 3 clusters: c0 Monotonic_down (~25%), c1 Choppy (~52%), "
        "c2 V-shape (~23%). Under 5ers_eet + multi_tf restored + W1 canonical, the K-sweep "
        "and cluster boundaries are re-determined from scratch on path-shape features "
        "(which are R-unit, signal-pool-derived — independent of bar-storage convention). "
        "Material archetype shifts are flagged below.",
        "",
        "## Silhouette by K",
        "",
