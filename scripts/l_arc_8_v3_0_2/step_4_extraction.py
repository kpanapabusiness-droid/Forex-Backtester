...
def _build_summary_md(
    ...
    lines = [
        ...
        f"_Generated: {datetime.now(timezone.utc).isoformat()}Z_",
        "",
        f"- Engine path: `core.steps.step_4_extraction.run_step_4` (canonical, PR #185)",
        ...
        f"- Classifiers: RF + LGBM + Logistic at Appendix A defaults",
    ...
    lines += [
        ...
        "",
        f"- Arc 8 v3.0 (UTC + multi_tf all-NaN) c2 V-shape RF mean OOS AUC: 0.5300. NOT a baseline — different methodology.",
        f"- Prior `arc/l_arc_8_v3.0.2_halted` (UTC + W1 leaking) c2 V-shape mean AUC: 0.6938. NOT a baseline — driven by lookahead per verification doc §1.3.",
        f"- v3.0.2 (5ers_eet + canonical W1) per-cluster AUCs above are the canonical observation under this methodology.",
        "",
