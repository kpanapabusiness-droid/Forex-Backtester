...
def load_and_join(verbose: bool = True) -> pd.DataFrame:
    ...
    merged = ti.merge(sf, on=["pair", "signal_bar_ts"], how="left", validate="one_to_one")
    merged = merged.merge(bm, on="trade_id", how="left", validate="one_to_one")
    merged = merged.sort_values("trade_id").reset_index(drop=True)

    if len(merged) != 3993:
        raise RuntimeError(f"HALT - merged taken row count {len(merged)} != 3993")
...
def assign_quintiles(taken: pd.DataFrame) -> pd.DataFrame:
    qa_labels, _ = make_quintile_labels(taken["concurrent_signals_same_bar"], taken["trade_id"])
    qb_labels, _ = make_quintile_labels(taken["dist_d1_kijun_atr"], taken["trade_id"])
    taken = taken.copy()
    ...
    for q, exp in EXPECTED_Q_SIZES_BY_Q.items():
        if qa_counts.get(q, 0) != exp:
            raise RuntimeError(
                f"HALT - Q_A_concurrent marginal {q} = {qa_counts.get(q, 0)}, expected {exp}"
            )
        if qb_counts.get(q, 0) != exp:
            raise RuntimeError(
                f"HALT - Q_B_dist_d1 marginal {q} = {qb_counts.get(q, 0)}, expected {exp}"
            )
    return taken
...
def gate_2_1_cell_counts(taken: pd.DataFrame) -> Dict[Tuple[str, str], int]:
    ...
    expected = {
        (row["Q_A_concurrent"], row["Q_B_dist_d1"]): int(row["n"]) for _, row in block_p.iterrows()
    }
    observed: Dict[Tuple[str, str], int] = {}
    diffs: List[str] = []
    for qa in Q_ORDER:
        for qb in Q_ORDER:
            n = int(((taken["Q_A_concurrent"] == qa) & (taken["Q_B_dist_d1"] == qb)).sum())
            observed[(qa, qb)] = n
...
# ---------------------------------------------------------------------------
def assign_path_categories(per_bar: pd.DataFrame, n_trades: int) -> np.ndarray:
...
# ---------------------------------------------------------------------------
def compute_block_V(taken: pd.DataFrame, cats: np.ndarray) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for subset_id, defn in SUBSET_DEFS:
        ...
        for c in ALL_CATS:
            ...
            pct_of_population = n_cat / pop_n_cat if pop_n_cat else float("nan")
            over_rep = pct_of_subset / pop_pct_by_cat[c] if pop_pct_by_cat[c] > 0 else float("nan")
            if n_cat > 0:
            ...
            contrib = pct_of_subset * mean_R_within if (n_cat > 0 and n_subset > 0) else 0.0
            ...
            rows_V.append(
                {
                    ...
                    "mean_R_diff_vs_pop": (
                        mean_R_within - pop_mean_R_by_cat[c] if n_cat > 0 else float("nan")
                    ),
        ...
        consistency_diff = (
            float(pooled - expected_pooled) if expected_pooled is not None else float("nan")
        )
        ...
        rows_summary.append(
            {
                ...
                "pooled_mean_R_from_block_P": expected_pooled
                if expected_pooled is not None
                else float("nan"),
...
# ---------------------------------------------------------------------------
def compute_block_W(taken: pd.DataFrame, cats: np.ndarray, per_bar: pd.DataFrame) -> pd.DataFrame:
...
# ---------------------------------------------------------------------------
def _build_close_matrix(per_bar: pd.DataFrame, n_trades: int) -> Tuple[np.ndarray, np.ndarray]:
    """Returns:
...
def _category_segments(mat: np.ndarray, tids_in_cat: np.ndarray) -> List[np.ndarray]:
...
def _plot_category_panel(
    ...
    if n_cat == 0:
        _style_axes(ax)
        ax.set_title(f"{cat} (n=0) - Subset {subset_id} - Block B at 1R", fontsize=title_size)
        return
...
def write_report(
    ...
    for subset_id, _ in SUBSET_DEFS:
        ...
        sub = sub.sort_values("abs_contrib", ascending=False).drop(columns="abs_contrib")
        sub = sub[cols_order]
    ...
    for subset_id, _ in SUBSET_DEFS:
        ...
        piv = sub.pivot_table(
            index="category",
            columns="k",
            values="median_close_R",
            aggfunc="first",
        )
        # Add pop reference row (S0_pop) for comparison
        pop_sub = df_W[(df_W["subset_id"] == "S0_pop") & (df_W["k"].isin(sel_ks))]
    ...
    L.append("")
    L.append(
        "- The spec section 3 path-category text used a +1.0 ATR threshold,"
    ...
    L.append(
        "- The spec mentions a `running_close_atr` column; the actual"
    ...
    L.append("")
    pop_pooled = float(df_summary.loc[df_summary["subset_id"] == "S0_pop", "pooled_mean_R"].iloc[0])
    for subset_id, _ in SUBSET_DEFS:
        if subset_id == "S0_pop":
            continue
        sub = df_V[df_V["subset_id"] == subset_id].copy()
    ...
    onlyup_rep = df_V[(df_V["category"] == "only_up") & (df_V["subset_id"] != "S0_pop")]
    onlyup_rep_sorted = onlyup_rep.sort_values("over_representation", ascending=False)
    if not onlyup_rep_sorted.empty:
        ...
        pop_pct_only_up = (
            ...
            if top_ou["over_representation"] > 0
            else float("nan")
        )
        L.append(
    ...
    stsl_share = df_V[(df_V["category"] == "straight_to_sl") & (df_V["subset_id"] != "S0_pop")]
    ...
    for _, yr in df_Y.iterrows():
        ch = (
            "mix-shift"
            if abs(yr["mix_shift_component"]) > abs(yr["per_cat_shift_component"])
...
def build_pass(out_dir: Path, *, verbose: bool = True) -> Dict[str, str]:
    ...
    for subset_id, defn in SUBSET_DEFS:
        ...
        if n != defn["expected_n"]:
            raise RuntimeError(f"HALT (gate 2.2): {subset_id} n={n} != {defn['expected_n']}")
    ...
    pb = pd.read_csv(
        ...
        dtype={
            "trade_id": np.int64,
            "k": np.int32,
            "running_mfe_atr": np.float64,
            "running_mae_atr": np.float64,
    ...
    if viol:
        details = "\n".join(f"  L{ln}: pattern '{pat}' in: {txt}" for ln, pat, txt in viol)
        raise RuntimeError(
...
def main() -> int:
    ...
    if not args.single_pass:
        ...
        print(
            f"  CSV/MD: all {sum(1 for k in shas_A if k.endswith('.csv') or k.endswith('.md'))} files byte-identical",
            flush=True,
        )
        ...
        build_pass(out_dir, verbose=True)
    ...
    staged_out = (
        subprocess.check_output(["git", "diff", "--name-only", "--cached"], cwd=str(REPO_ROOT))
        .decode()
        .strip()
    )
    if staged_out:
    ...
    print(f"[Done] wallclock={elapsed:.2f}s peak_python_alloc={peak_mb:.1f} MB", flush=True)
    return 0
