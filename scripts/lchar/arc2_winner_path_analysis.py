...
def load_inputs() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if n_nan_k6 / len(clust) > 0.01:
        raise RuntimeError(
            f"BLOCKER: km_k6 invalid for >1% rows ({n_nan_k6} of {len(clust)})"
        )
...
def analysis_1_view_A(panel: pd.DataFrame, shape_c: pd.DataFrame) -> pd.DataFrame:
    ...
    panel_a = panel[panel["t"] <= 60][["signal_idx", "t", "r_close",
                                       "running_mfe", "running_mae"]].copy()
    panel_a = panel_a.merge(shape_c[["signal_idx", "km_k6"]],
...
def analysis_1_view_B(panel: pd.DataFrame, shape_c: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    print("[6/9] Analysis 1 View B — run-up decile envelopes per cluster...",
          flush=True)

    # Pick peak_bar per trade per spec.
    sc = shape_c.copy()
    sc["peak_bar_for_view_B"] = np.where(
        sc["km_k6"].isin(WINNER_CLUSTERS), sc["peak_mfe_bar"],
        np.where(sc["km_k6"].isin(LOSER_CLUSTERS), sc["peak_mae_bar"],
                 np.where(sc["final_r"] > 0.0, sc["peak_mfe_bar"],
                          sc["peak_mae_bar"]))
    ).astype(np.int64)
    ...
    print(f"   note: {n_dropped} trades dropped from View B (peak_bar < 1)",
          flush=True)

    panel_b = panel[["signal_idx", "t", "r_close",
    ...
    peak_map = dict(zip(sc_valid["signal_idx"].to_numpy(),
                        sc_valid["peak_bar_for_view_B"].to_numpy()))
    cluster_map = dict(zip(sc_valid["signal_idx"].to_numpy(),
                           sc_valid["km_k6"].to_numpy()))
...
def analysis_2_per_bar(panel: pd.DataFrame, shape_c: pd.DataFrame) -> pd.DataFrame:
    ...
    panel_sub = panel_sub.merge(shape_c[["signal_idx", "km_k6"]],
                                on="signal_idx", how="inner")
    ...
    for c in CLUSTERS:
        for bar in bars:
            ...
            for i, count in enumerate(hist):
                ...
                rows.append({**row_base, "stat": f"hist_{bin_label}",
                             "value": float(count)})
            # Also overflow counts (vals outside [-3, +3]).
            below = int((vals < -3.0).sum())
            above = int((vals > 3.0).sum())
            rows.append({**row_base, "stat": "hist_below_neg3R",
                         "value": float(below)})
            rows.append({**row_base, "stat": "hist_above_pos3R",
                         "value": float(above)})
...
def analysis_3_separation(panel: pd.DataFrame,
                          shape_c: pd.DataFrame) -> Tuple[pd.DataFrame, dict]:
    """ROC AUC of single-feature and composite classifiers per bar."""
    print("[8/9] Analysis 3 — separation diagnostics (winner vs loser)...",
          flush=True)
    ...
    panel_sub = panel_sub.merge(sc[["signal_idx", "is_winner"]],
                                on="signal_idx", how="inner")
    ...
    for bar in SEP_BARS:
        ...
        for metric in metrics:
            ...
            rows.extend([
                {"bar": int(bar), "classifier_type": f"single_{metric}",
                 "metric": "auc", "value": auc},
                {"bar": int(bar), "classifier_type": f"single_{metric}",
                 "metric": "threshold", "value": t_star},
                {"bar": int(bar), "classifier_type": f"single_{metric}",
                 "metric": "sensitivity_at_t_star", "value": sens},
                {"bar": int(bar), "classifier_type": f"single_{metric}",
                 "metric": "specificity_at_t_star", "value": spec},
                {"bar": int(bar), "classifier_type": f"single_{metric}",
                 "metric": "balanced_accuracy_at_t_star",
                 "value": 0.5 * (sens + spec)},
                {"bar": int(bar), "classifier_type": f"single_{metric}",
                 "metric": "n", "value": float(len(y))},
            ])
        ...
        lr = LogisticRegression(
            solver="lbfgs", random_state=SKLEARN_SEED, max_iter=1000
        )
        lr.fit(X_tr, y_tr)
        ...
        rows.extend([
            {"bar": int(bar), "classifier_type": "composite_logreg",
             "metric": "auc_train", "value": auc_tr},
            {"bar": int(bar), "classifier_type": "composite_logreg",
             "metric": "auc_test", "value": auc_te},
            {"bar": int(bar), "classifier_type": "composite_logreg",
             "metric": "auc_train_test_gap", "value": auc_tr - auc_te},
            {"bar": int(bar), "classifier_type": "composite_logreg",
             "metric": "coef_r_close", "value": float(coefs[0])},
            {"bar": int(bar), "classifier_type": "composite_logreg",
             "metric": "coef_running_mfe", "value": float(coefs[1])},
            {"bar": int(bar), "classifier_type": "composite_logreg",
             "metric": "coef_running_mae", "value": float(coefs[2])},
            {"bar": int(bar), "classifier_type": "composite_logreg",
             "metric": "intercept", "value": intercept},
            {"bar": int(bar), "classifier_type": "composite_logreg",
             "metric": "n_train", "value": float(len(y_tr))},
            {"bar": int(bar), "classifier_type": "composite_logreg",
             "metric": "n_test", "value": float(len(y_te))},
        ])
...
def _running_peak_pullbacks(rl: np.ndarray, rh: np.ndarray,
                            peak_bar: int) -> Dict[str, float]:
...
def analysis_4_winner_pullbacks(panel: pd.DataFrame,
                                shape_c: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    panel_w = panel[panel["signal_idx"].isin(winners["signal_idx"])][
        ["signal_idx", "t", "r_high", "r_low"]
    ].sort_values(["signal_idx", "t"], kind="mergesort").reset_index(drop=True)

    per_trade_rows: List[Dict[str, object]] = []
    peak_map = dict(zip(winners["signal_idx"].to_numpy(),
                        winners["peak_mfe_bar"].to_numpy()))
    cluster_map = dict(zip(winners["signal_idx"].to_numpy(),
                           winners["km_k6"].to_numpy()))
    ...
    def agg_block(sub: pd.DataFrame, label: str) -> List[Dict[str, object]]:
        ...
        for k, v in stats.items():
            out.append({"group": label, "metric": "max_dd_from_running_peak",
                        "stat": k, "value": v})
        # Mean/median pullback counts.
        for th_col in ("n_pullbacks_ge_0.25R", "n_pullbacks_ge_0.5R",
                       "n_pullbacks_ge_1R", "n_pullbacks_ge_1.5R"):
            cv = sub[th_col].to_numpy()
            out.append({"group": label, "metric": th_col, "stat": "mean",
                        "value": float(np.mean(cv))})
            out.append({"group": label, "metric": th_col, "stat": "median",
                        "value": float(np.median(cv))})
            out.append({"group": label, "metric": th_col, "stat": "p90",
                        "value": float(np.percentile(cv, 90))})
        # Near-peak giveback.
        npv = sub["pullback_at_peak_minus_5"].to_numpy()
        out.append({"group": label, "metric": "pullback_at_peak_minus_5",
                    "stat": "mean", "value": float(np.mean(npv))})
        out.append({"group": label, "metric": "pullback_at_peak_minus_5",
                    "stat": "median", "value": float(np.median(npv))})
        out.append({"group": label, "metric": "pullback_at_peak_minus_5",
                    "stat": "p90", "value": float(np.percentile(npv, 90))})
        ...
        for i, count in enumerate(hist):
            ...
            out.append({"group": label, "metric": "max_dd_from_running_peak",
                        "stat": f"hist_{label_h}", "value": float(count)})
        return out
...
def analysis_5_time_to_band(panel: pd.DataFrame,
                            shape_c: pd.DataFrame) -> pd.DataFrame:
    ...
    bands = MFE_BANDS
    sig_band: Dict[Tuple[int, float], int] = {}

    # Vectorised: for each band X, group by signal_idx and find min t where
    # running_mfe >= X.
    grouped = panel_sub.groupby("signal_idx", sort=True)
    ...
    for X in bands:
        ...
        if mask.any():
            sub_first = panel_sub.loc[mask, ["signal_idx", "t"]] \
                                 .groupby("signal_idx", sort=True)["t"].min()
        else:
            sub_first = pd.Series(dtype=np.int64)
        sig_first_times[X] = sub_first

    rows: List[Dict[str, object]] = []
    sig_cluster = shape_c.set_index("signal_idx")["km_k6"].to_dict()
    ...
    for c in CLUSTERS:
        ...
        for X in bands:
            ...
            if n_reached == 0:
                row_base = {"cluster": int(c), "band_R": float(X),
                            "n_cluster": int(n_c)}
                rows.append({**row_base, "stat": "n_reached", "value": 0.0})
                rows.append({**row_base, "stat": "p_reached",
                             "value": 0.0})
                continue
            vals = first_times.dropna().to_numpy()
            row_base = {"cluster": int(c), "band_R": float(X),
                        "n_cluster": int(n_c)}
            rows.append({**row_base, "stat": "n_reached",
                         "value": float(n_reached)})
            rows.append({**row_base, "stat": "p_reached",
                         "value": float(n_reached) / float(n_c)})
            rows.append({**row_base, "stat": "mean",
                         "value": float(np.mean(vals))})
            rows.append({**row_base, "stat": "std",
                         "value": float(np.std(vals, ddof=1)) if n_reached > 1 else 0.0})
            for pct in (5, 10, 25, 50, 75, 90, 95):
                rows.append({**row_base, "stat": f"p{pct:02d}",
                             "value": float(np.percentile(vals, pct))})
...
def _classify_winner_shape(rl: np.ndarray, rh: np.ndarray, rc: np.ndarray,
                           rmfe: np.ndarray, rmae: np.ndarray,
                           peak_bar: int) -> Tuple[str, float, int]:
    ...
    v_shape = (
        rmae_05_bar >= 0
        and (rmfe_pos05_bar < 0 or rmae_05_bar < rmfe_pos05_bar)
    )
...
def analysis_6_shape(panel: pd.DataFrame,
                     shape_c: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    panel_w = panel[panel["signal_idx"].isin(interest["signal_idx"])][
        ["signal_idx", "t", "r_close", "r_high", "r_low",
         "running_mfe", "running_mae"]
    ].sort_values(["signal_idx", "t"], kind="mergesort").reset_index(drop=True)
    ...
    for sid, cl, pmf, pma, fr in zip(
        interest["signal_idx"], interest["km_k6"],
        interest["peak_mfe_bar"], interest["peak_mae_bar"],
        interest["final_r"]
    ):
        peak_map[int(sid)] = int(pmf) if cl in (1, 2, 4, 5) and (cl in (2, 5) or fr > 0) else int(pma)
    ...
    for c in (1, 2, 4, 5):
        ...
        for sh in shape_order:
            ...
            cross_rows.append({
                ...
                "mean_max_dd_from_running_peak":
                    float(cell["max_dd_from_running_peak"].mean()) if n > 0 else 0.0,
            })
...
def chart_view_A_overlay(df: pd.DataFrame) -> None:
    ...
    for c in CLUSTERS:
        ...
        ax.plot(s["bar"], s["value"], label=f"c{c}: {CLUSTER_LABELS[c]}",
                color=CLUSTER_COLORS[c], linewidth=1.6)
...
def chart_view_A_grid(df: pd.DataFrame) -> None:
    ...
    for idx, c in enumerate(CLUSTERS):
        ...
        for pct in PERCENTILES:
            s = df[(df["cluster"] == c) & (df["metric"] == "r_close")
                   & (df["percentile"] == pct)].sort_values("bar")
            ax.plot(s["bar"], s["value"], label=f"p{int(pct)}",
    ...
    fig.suptitle("View A — p10/p25/p50/p75/p90 of r_close[t] by cluster, bars 0..60",
                 fontsize=11)
    fig.tight_layout()
...
def chart_view_B_overlay(df: pd.DataFrame) -> None:
    ...
    for c in CLUSTERS:
        ...
        ax.plot(s["decile"], s["value"], label=f"c{c}: {CLUSTER_LABELS[c]}",
                color=CLUSTER_COLORS[c], linewidth=1.6, marker="o", ms=3)
    ax.axhline(0.0, color="black", linewidth=0.6, linestyle=":")
...
def chart_view_B_grid(df: pd.DataFrame) -> None:
    ...
    for idx, c in enumerate(CLUSTERS):
        ...
        for pct in PERCENTILES:
            s = df[(df["cluster"] == c) & (df["metric"] == "r_close")
                   & (df["percentile"] == pct)].sort_values("decile")
            ax.plot(s["decile"], s["value"], label=f"p{int(pct)}",
                    color=CLUSTER_COLORS[c],
                    alpha=0.3 + 0.13 * PERCENTILES.index(pct),
                    linewidth=1.0, marker="o", ms=2)
        ax.axhline(0.0, color="black", linewidth=0.5, linestyle=":")
        ax.set_title(f"c{c}: {CLUSTER_LABELS[c]}", fontsize=10)
        ax.grid(True, linewidth=0.4, alpha=0.5)
    for ax in axes[1]:
    ...
    for ax in axes[:, 0]:
        ax.set_ylabel("r_close (R)")
    fig.suptitle("View B — p10/p25/p50/p75/p90 of r_close at run-up deciles",
                 fontsize=11)
    fig.tight_layout()
...
def chart_auc_vs_bar(sep_df: pd.DataFrame) -> None:
    ...
    for c_type, label, color, metric in types_to_plot:
        s = sep_df[(sep_df["classifier_type"] == c_type)
                   & (sep_df["metric"] == metric)].sort_values("bar")
...
def chart_threshold_curves(sep_df: pd.DataFrame) -> None:
    fig, ax = plt.subplots(figsize=(10, 6))
    s_t = sep_df[(sep_df["classifier_type"] == "single_r_close")
                 & (sep_df["metric"] == "threshold")].sort_values("bar")
    s_ba = sep_df[(sep_df["classifier_type"] == "single_r_close")
                  & (sep_df["metric"] == "balanced_accuracy_at_t_star")
                  ].sort_values("bar")
    ax2 = ax.twinx()
    ax.plot(s_t["bar"], s_t["value"], color="#1f77b4", label="T* (R)",
            linewidth=1.4)
    ax2.plot(s_ba["bar"], s_ba["value"], color="#d62728",
             label="balanced acc @ T*", linewidth=1.4)
    ax.set_xlabel("bar t")
    ...
    ax.grid(True, linewidth=0.4, alpha=0.5)
    fig.suptitle("Single-feature r_close[t] — Youden-J threshold and balanced "
...
def chart_winner_pullbacks(per_trade: pd.DataFrame) -> None:
    ...
    for c, color, ax_idx in ((2, CLUSTER_COLORS[2], 0), (5, CLUSTER_COLORS[5], 1)):
        ...
        ax.axvline(float(vals.median()), color="black", linewidth=1.0,
                   linestyle="--", label=f"median={vals.median():.2f}R")
        ax.axvline(float(np.percentile(vals, 95)), color="grey",
...
def write_report(
    ...
    lines.append(f"Total n with valid km_k6: **{n_total}** "
    ...
    lines.append("")
    lines.append("Median r_close[t] per cluster at key early-bar checkpoints "
                 "(values in R = 2 × Wilder ATR(14) at bar N close).")
    lines.append("")
    cols = ["bar"] + [f"c{c}" for c in CLUSTERS]
    lines.append("| " + " | ".join(cols) + " |")
    lines.append("|" + "|".join(["---"] * (len(CLUSTERS) + 1)) + "|")
    medians_at_check = view_A[(view_A["metric"] == "r_close")
                              & (view_A["percentile"] == 50.0)]
    for bar in PER_BAR_CHECKPOINTS:
        row = [str(bar)]
        for c in CLUSTERS:
            v = float(medians_at_check[
                (medians_at_check["cluster"] == c)
                & (medians_at_check["bar"] == bar)
            ]["value"].iloc[0])
            row.append(f"{v:+.3f}")
    ...
    lines.append("")
    lines.append("r_close[t] mean and percentiles at bars {6, 12, 24, 36, 48, 60}, "
                 "per cluster:")
    lines.append("")
    for bar in PER_BAR_CHECKPOINTS:
    ...
    lines.append("")
    lines.append("Each trade's panel sliced t=0..peak_bar (peak_mfe_bar for "
    ...
    lines.append("| " + " | ".join(cols) + " |")
    lines.append("|" + "|".join(["---"] * (len(CLUSTERS) + 1)) + "|")
    med_B = view_B[(view_B["metric"] == "r_close")
                   & (view_B["percentile"] == 50.0)]
    for d in DECILES:
        row = [f"{d:.1f}"]
        for c in CLUSTERS:
            v = float(med_B[
                (med_B["cluster"] == c) & (np.isclose(med_B["decile"], d))
            ]["value"].iloc[0])
            row.append(f"{v:+.3f}")
        lines.append("| " + " | ".join(row) + " |")
    lines.append("")
    ...
    lines.append("")
    lines.append("Population: winners (c2 ∪ c5) and losers (c3 ∪ c6); middle "
                 "clusters (c1, c4) excluded from this analysis.")
    lines.append("")
    lines.append("AUC trace per bar for single-feature and composite classifiers:")
    lines.append("")
    lines.append("| bar | AUC r_close | AUC running_mfe | AUC running_mae | "
    ...
    for bar in (1, 6, 12, 18, 24, 30, 36, 42, 48, 54, 60):
        def get_metric(c_type: str, metric_name: str) -> float:
            row = sep[(sep["classifier_type"] == c_type)
                      & (sep["bar"] == bar)
    ...
    auc_rclose = sep[(sep["classifier_type"] == "single_r_close")
                     & (sep["metric"] == "auc")].sort_values("bar")
    auc_at_b = dict(zip(auc_rclose["bar"].astype(int).tolist(),
                        auc_rclose["value"].tolist()))
    def first_bar_cross(threshold: float) -> int:
        for b in sorted(auc_at_b):
            if auc_at_b[b] >= threshold:
                return b
        return -1
    lines.append("Single-feature AUC of r_close[t] discriminating winners from "
                 "losers crosses:")
    lines.append("")
    for th in (0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90):
        b = first_bar_cross(th)
        if b > 0:
            lines.append(f"- AUC ≥ {th:.2f} at bar **{b}** "
                         f"(AUC[{b}] = {auc_at_b[b]:.3f})")
    lines.append("")

    # Optimal thresholds at key bars.
    lines.append("Youden-J optimal thresholds for single-feature r_close[t] "
                 "classifier at key bars:")
    lines.append("")
    lines.append("| bar | AUC | T* (R) | sensitivity | specificity | "
                 "balanced acc |")
    lines.append("|---:|---:|---:|---:|---:|---:|")
    for bar in (6, 12, 24, 36, 48, 60):
        def get(metric: str) -> float:
            return float(sep[(sep["classifier_type"] == "single_r_close")
                             & (sep["bar"] == bar)
                             & (sep["metric"] == metric)]["value"].iloc[0])
        lines.append(
    ...
    lines.append("Composite logistic regression coefficients at key bars "
                 "(features: r_close, running_mfe, running_mae):")
    lines.append("")
    lines.append("| bar | β r_close | β running_mfe | β running_mae | intercept | "
    ...
    max_gap = max(abs(d["auc_train"] - d["auc_test"])
                  for d in composite_info.values())
    warn_train_test = max_gap > 0.05
    if warn_train_test:
        lines.append(f"**WARN**: composite classifier max |train−test AUC gap| "
                     f"= {max_gap:.4f} across bars 1..60 (threshold = 0.05). "
                     f"Possible overfitting at one or more bars.")
    else:
        lines.append(f"Train-test AUC gap maximum across bars 1..60 = "
    ...
    lines.append("Pullback event counts (mean / median) within run-up, by "
                 "threshold:")
    lines.append("")
    lines.append("| group | ≥0.25R mean | ≥0.25R median | ≥0.5R mean | ≥0.5R median | "
                 "≥1R mean | ≥1R median | ≥1.5R mean | ≥1.5R median |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|")
    for group in ("cluster_2", "cluster_5", "cluster_2_and_5"):
        def cnt(metric: str, stat: str) -> float:
            row = pullback_agg[(pullback_agg["group"] == group)
    ...
    hist_rows = pullback_agg[(pullback_agg["group"] == "cluster_2_and_5")
                             & (pullback_agg["metric"] == "max_dd_from_running_peak")
    ...
    n_combined = float(pullback_agg[(pullback_agg["group"] == "cluster_2_and_5")
                                    & (pullback_agg["metric"] == "max_dd_from_running_peak")
                                    & (pullback_agg["stat"] == "n")]["value"].iloc[0])
    frac_overflow = n_overflow / n_combined if n_combined > 0 else 0.0
    if frac_overflow > 0.05:
        lines.append(f"**WARN**: pullback histogram overflow "
                     f"({frac_overflow:.1%}) > 5%; some max_dd values exceed 5R.")
    else:
        lines.append(f"Pullback histogram overflow (combined winners) = "
                     f"{frac_overflow:.2%} (≤ 5%).")
    lines.append("")
    ...
    for X in MFE_BANDS:
        ...
        lines.append("")
        lines.append("| cluster | n_cluster | n_reached | p_reached | mean bar | "
                     "p10 | p25 | p50 | p75 | p90 | p_reach_next_band |")
        lines.append("|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
        for c in CLUSTERS:
            sub = time_to_band[(time_to_band["cluster"] == c)
    ...
    lines.append("")
    lines.append("Shape buckets (mutually exclusive, evaluated in order: "
    ...
    lines.append("")
    lines.append("| cluster | shape | n | frac_of_cluster | mean peak_mfe_r | "
    ...
    if empty_cells:
        ...
        lines.append("")
    if warn_low_cells:
        lines.append(f"**WARN**: low-confidence sampling cells "
                     f"(0 < n < 30) in winners: "
    ...
    def shape_frac(c: int, sh: str) -> float:
        row = shape_cross[(shape_cross["cluster"] == c)
                          & (shape_cross["shape"] == sh)]
        if len(row) == 0:
            return 0.0
    ...
    def time_stat(c: int, X: float, stat: str) -> float:
        if stat == "n_cluster":
            row = time_to_band[(time_to_band["cluster"] == c)
                               & (time_to_band["band_R"] == X)]
            if len(row) == 0:
                return float("nan")
...
def maybe_append_candidate(
    sep: pd.DataFrame, pullback_agg: pd.DataFrame, shape_cross: pd.DataFrame,
    composite_info: dict,
) -> None:
    ...
    def pull(stat: str, group: str = "cluster_5") -> float:
        return float(pullback_agg[(pullback_agg["group"] == group)
                                  & (pullback_agg["metric"] == "max_dd_from_running_peak")
                                  & (pullback_agg["stat"] == stat)]["value"].iloc[0])
    ...
    def get_at(metric: str, bar: int) -> float:
        return float(sep[(sep["classifier_type"] == "single_r_close")
                         & (sep["bar"] == bar)
                         & (sep["metric"] == metric)]["value"].iloc[0])
    new_section.append("Composite logistic-regression on (r_close[t], "
    ...
    new_section.append("")
    new_section.append("**Effect size:** Youden-J single-feature threshold "
    ...
    new_section.append("")
    new_section.append("**Pre-committed fresh-arc gate (sketch — full spec is "
                       "the new arc-opening doc's job):**")
    new_section.append("- Mid-trade decision: at bar t* (pre-committed), apply "
    ...
    new_section.append("- Tunables: t* ∈ {12, 24, 36, 48}, threshold rule ∈ "
                       "{single-feature T*, composite p<0.5}, pre-committed "
                       "before the WFO.")
    new_section.append("- Gate per L6.0 §4: worst-fold ROI > 0%, worst-fold "
                       "DD < 8%, trades-per-fold ≥ 15.")
    new_section.append("")
    new_section.append("**n:** ~27,455 (winners + losers; middle clusters "
                       "are ambiguous and would be handled by the rule directly).")
    new_section.append("**Priority:** HIGH if AUC ≥ 0.75 stays at bar t ≤ 24. "
                       "MEDIUM if only achievable at bar t ≥ 48 (signal arrives "
                       "late; capital is tied up longer; less actionable).")
    new_section.append("**Caveats:** AUC is computed on the cluster labels, "
    ...
    new_section.append("---")
    new_section.append("")
    new_section.append("## Candidate 9 — Winner-specific trail sized to "
                       "cluster-5 pullback distribution")
    new_section.append("")
    new_section.append("**Observation (from `winner_pullbacks.csv` and "
                       "Analysis 4):**")
    new_section.append("")
    new_section.append(
    ...
    new_section.append("- Exit rule: after first-touch of `running_mfe ≥ A × R`, "
    ...
    new_section.append("")
    new_section.append("**n:** Full 41,794 after the rule is applied; cluster "
                       "5 (n=4,787) is the population where the giveback width "
                       "matters most.")
    new_section.append("**Priority:** MEDIUM. The trail is the most-tested "
                       ...
                       "a wide trail.")
    new_section.append("**Caveats:** Wide trails (G ≥ 1.5R) on cluster-3/6 "
    ...
    new_section.append("---")
    new_section.append("")
    new_section.append("## Candidate 10 — Cluster-aware classifier feature set "
                       "for Candidate 4")
    new_section.append("")
    new_section.append("**Observation (from Analysis 3 composite logistic "
                       "regression):**")
    new_section.append("")
    new_section.append(
    ...
    new_section.append("")
    new_section.append("**Pre-committed fresh-arc gate (sketch):**")
    new_section.append("- Extend Candidate 4's ex-ante classifier feature set "
    ...
    new_section.append("- Gate per L6.0 §4.")
    new_section.append("")
    new_section.append("**n:** N depends on the bar t* chosen; for t* ≤ 60, "
                       "n = 41,794 (all signals reach bar t* in the panel).")
    new_section.append("**Priority:** MEDIUM. Architectural — depends on "
                       "Candidate 4 being run first. The mid-trade features "
                       "are cheap to add to a Candidate 4 sketch.")
    new_section.append("**Caveats:** Cluster labels are post-hoc; AUC here "
...
def run_all() -> Dict[str, str]:
    ...
    write_csv_deterministic(view_A.sort_values(
        ["cluster", "bar", "metric", "percentile"]), OUTPUT_DIR / "cluster_envelopes_view_A.csv")
    write_csv_deterministic(view_B.sort_values(
        ["cluster", "decile", "metric", "percentile"]), OUTPUT_DIR / "cluster_envelopes_view_B.csv")
    write_csv_deterministic(per_bar.sort_values(
        ["cluster", "bar", "metric", "stat"]), OUTPUT_DIR / "per_bar_distributions.csv")
    write_csv_deterministic(sep.sort_values(
        ["bar", "classifier_type", "metric"]), OUTPUT_DIR / "separation_diagnostics.csv")
    ...
    write_csv_deterministic(pullback_agg.sort_values(
        ["group", "metric", "stat"]), OUTPUT_DIR / "winner_pullbacks.csv")
    write_csv_deterministic(time_to_band.sort_values(
        ["cluster", "band_R", "stat"]), OUTPUT_DIR / "time_to_band_per_cluster.csv")
    write_csv_deterministic(shape_cross.sort_values(
        ["cluster", "shape"]), OUTPUT_DIR / "winner_shape_classification.csv")
    ...
    report_text, warn_gap = write_report(
        panel, shape_c, view_A, view_B, per_bar, sep,
        pullback_per_trade, pullback_agg, time_to_band, shape_cross,
        composite_info,
    )
    banned = grep_banned(report_text)
    if banned:
        raise RuntimeError(
            f"BLOCKER: report contains banned §14.5 patterns: {banned}"
        )
    (OUTPUT_DIR / "winner_path_report.md").write_bytes(
...
def main() -> int:
    ...
    lines.append("- Logistic regression: sklearn solver='lbfgs', "
    ...
    lines.append("- Matplotlib: Agg backend, metadata pinned to "
                 "Creation Time = 2020-01-01T00:00:00+00:00.")
    lines.append("")
    ...
    if diff:
        ...
        if non_png_diff or any_pixel_diff:
            ...
            lines.append("Overall: PASS (PNG pixel-hash byte-identical; "
