...
| v1.3.1+ (Amendment 3.1) | 2026-05-25 | `r_max` reframed as deployment cap. New optional fields: `r_safe_intrinsic_pct`, `r_hard_intrinsic_pct`, `r_safe_capped_at_rmax`, `r_hard_capped_at_rmax`. |
...
**Amendment 3.1 (2026-05-25 — r_max as deployment cap):** post-`AMENDMENT_3_1_CUTOFF_ISO` closures with `r_safe_capped_at_rmax: true` or `r_hard_capped_at_rmax: true` MUST also populate `r_safe_intrinsic_pct` / `r_hard_intrinsic_pct` for audit. Phase 1: WARNING-level check. Phase 2: ERROR after cutoff backfill.
