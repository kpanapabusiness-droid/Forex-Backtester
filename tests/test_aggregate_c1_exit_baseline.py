...
class TestAggregateC1ExitBaseline:
    ...
    def test_aggregate_baseline_uses_real_pair_column(self):
        ...
        batch_data = {
            ...
            "roles": [
                json.dumps(
                    {"c1": "coral", "c2": None, "baseline": None, "volume": None, "exit": None}
                ),
                json.dumps(
                    {"c1": "aso", "c2": None, "baseline": None, "volume": None, "exit": None}
                ),
            ],
            "params": [
    ...
    def test_csv_quality_gate_passes_with_one_bad_line(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with batch_csv.open("a", encoding="utf-8") as f:
                f.write(
                    "EUR_USD,coral,exit_twiggs_money_flow,none,100,50,30,20,10.5,5.0,0.5,extra,commas,here\n"
                )
    ...
    def test_csv_quality_gate_fails_with_many_bad_lines(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with batch_csv.open("a", encoding="utf-8") as f:
                ...
                f.write(
                    "EUR_USD,coral,exit_twiggs_money_flow,none,100,50,30,20,10.5,5.0,0.5,more,commas,here\n"
                )
                f.write(
                    "EUR_USD,coral,exit_twiggs_money_flow,none,100,50,30,20,10.5,5.0,0.5,even,more,commas\n"
                )
    ...
    def test_pair_level_aggregation_preserves_pairs(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert result_pairs == set(pairs), (
                f"Missing pairs. Expected {set(pairs)}, got {result_pairs}"
            )
            ...
            for pair_val in result_df["pair"].unique():
                assert "_" in pair_val, (
                    f"Pair '{pair_val}' should be FX pair format (e.g., EUR_USD)"
                )
