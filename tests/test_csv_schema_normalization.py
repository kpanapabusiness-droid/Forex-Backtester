...
def test_normalize_legacy_schema(legacy_csv_sample):
    ...
    assert list(result.columns[:6]) == expected_cols, (
        f"Expected {expected_cols}, got {list(result.columns[:6])}"
    )
...
def test_normalize_mt5_schema(mt5_csv_sample):
    ...
    assert (result["volume"] == mt5_csv_sample["tick_volume"]).all(), (
        "volume should equal tick_volume"
    )
...
def test_normalize_handles_missing_volume():
    ...
    assert (result["volume"] == 0.0).all() or result["volume"].isna().all(), (
        "missing volume should be 0 or NaN"
    )
...
def test_normalize_preserves_spread_extra_column(mt5_csv_sample):
    ...
    core_cols = ["date", "open", "high", "low", "close", "volume"]
    assert result.columns.get_loc("spread") > len(core_cols) - 1, (
        "spread should be after core columns"
    )
