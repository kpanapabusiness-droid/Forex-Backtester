...
class TestPhase1VolumeJoinKeys:
    ...
    def test_find_join_keys_rejects_too_few_keys(self):
        ...
        assert (
            "Insufficient join keys" in str(exc_info.value)
            or "at least 3" in str(exc_info.value).lower()
        )
    ...
    def test_find_join_keys_requires_pair_c1_exit(self):
        ...
        error_msg = str(exc_info.value)
        assert (
            "Missing required join keys" in error_msg
            or "pair" in error_msg.lower()
            or "c1" in error_msg.lower()
            or "exit" in error_msg.lower()
        )
